...
import { dataStorageSpecialIds } from '../../../data/item-data/_item-data-axis-point';
import { specialValues } from '../../../data/special-values';
import { Color } from '../../color';
import { _getCustomPropertiesSerializationInfoCore } from '../../custom-properties/_custom-properties-utils';
import { _customPropertiesMeta } from '../../custom-properties/custom-properties-metadata';
import { DataItemLink } from '../../data-item/data-item';
import { dataItemLinkSerializationsInfo } from '../../data-item/metadata/_data-item';
import { CustomItemWindowDefinition } from '../../data-item/window-definition/custom-item-calc-window-definition';
...
import { _baseInteractivityOptionsMeta, _dashboardItemInteractivityOptionsMeta, _drillDownInteractivityOptionsMeta, _masterFilterInteractivityOptionsMeta, DashboardItemInteractivityOptions, DashboardItemMasterFilterInteractivityOptions } from '../options/interactivity-options';
import { coloringOptions } from '../options/metadata/_coloring-options';
import { coloringDimensions, coloringMeasures, customDashboardItemSerializationsInfo } from './metadata/_custom-item';
import { SliceTable } from './slice-table';
var COLOR_MEASURE_ID = 'ColorMeasure';
export class CustomItem extends DataDashboardItem {
    constructor(_meta, modelJson = {}, serializer = new ModelSerializer()) {
        ...
        this._getDimensionValue = (dataRow, dataItemUniqueName) => {
            var value = this._getSpecialValue(dataRow, dataItemUniqueName, dataStorageSpecialIds.Value);
            if (value === null || value === undefined)
                value = this._getDimensionUniqueValue(dataRow, dataItemUniqueName);
            if (value === specialValues.nullValueGuid)
                value = null;
            return value;
        };
        this._getDimensionDisplayText = (dataRow, dataItemUniqueName) => {
            ...
            if (uniqueValue === specialValues.olapNullValueGuid)
                return undefined;
            if (displayText == null) {
        ...
        this.customBindings.forEach((binding) => {
            ...
            if (binding.array) {
                this[linkPropertyName] = deserializeArray(modelCustomMetadata[binding.propertyName], (item) => new DataItemLink(this, item, serializer));
                ...
                subscribeArrayChange(this[linkPropertyName], {
                    added: item => item.itemType(binding.dataItemType)
                });
            }
            else {
                this[binding.propertyName] = ko.observable();
                this._attachDataItem(this, linkPropertyName, new DataItemLink(this, modelCustomMetadata[binding.propertyName], serializer));
            }
            this.customMetadata[linkPropertyName] = this[linkPropertyName];
        });
        this.legacyCustomProperties.forEach(p => {
            this.addDisposable(this.customMetadata[p.propertyName] = this[p.propertyName] = ko.pureComputed({
        ...
        this.addDisposable(ko.computed(() => {
            ...
            this.customBindings.forEach(binding => {
                ...
                if (!sliceTable) {
                    sliceTable = new SliceTable(this, { '@ItemType': 'SliceTable', '@Name': binding.slice });
                ...
                if (binding.enableColoring) {
                    if (binding.dataItemType === 'Dimension') {
                        bindings.forEach(link => DataDashboardItem._addColoringMeta([link]));
                        if (binding.array) {
                            prop.subscribe(links => DataDashboardItem._addColoringMeta(links));
                        }
    ...
    static getPropertyInfo(p) {
        return deepExtend({ modelName: '@' + p.propertyName, category: PropertyCategory.ViewModel }, p);
    }
    ...
    static _getSerializationsInfo(bindings) {
        return (bindings || []).map(binding => {
            return {
                ...
                array: binding.array,
                info: !binding.array ? dataItemLinkSerializationsInfo : undefined
            };
        });
    }
    ...
    static _getInfo(meta, customItemType) {
        if (!meta) {
            ...
            return customDashboardItemSerializationsInfo.concat([staticCustomPropertiesInfo]);
        }
        const dynamicInfo = [];
        ...
        if (isFilterAllowed && isDrillDownAllowed) {
            dynamicInfo.push(_dashboardItemInteractivityOptionsMeta);
        }
        else if (isFilterAllowed) {
            dynamicInfo.push(_masterFilterInteractivityOptionsMeta);
        }
        else if (isDrillDownAllowed) {
            dynamicInfo.push(_drillDownInteractivityOptionsMeta);
        }
        else {
            dynamicInfo.push(_baseInteractivityOptionsMeta);
        }
        var hasColoringDimensions = CustomItem._hasColoringDimensions(bindings);
        if (hasColoringDimensions) {
            dynamicInfo.push(coloringDimensions);
        }
        var hasColoringMeasures = CustomItem._hasColoringMeasures(bindings);
        if (hasColoringMeasures) {
            dynamicInfo.push(coloringMeasures);
        }
        if (hasColoringDimensions || hasColoringMeasures) {
            dynamicInfo.push(coloringOptions);
        }
        return customDashboardItemSerializationsInfo
    ...
    _format(value, formatViewModel) {
        return !!formatViewModel ? format(value, formatViewModel) : value.toString();
    }
    _getServerText(dataRow, dataItemUniqueName) {
        return this._getSpecialValue(dataRow, dataItemUniqueName, dataStorageSpecialIds.DisplayText);
    }
    ...
    _getMasterFilterMode() {
        return this.interactivityOptions instanceof DashboardItemMasterFilterInteractivityOptions ? this.interactivityOptions.masterFilterMode() : 'None';
    }
    _getDrillDownEnabled() {
        return this.interactivityOptions instanceof DashboardItemInteractivityOptions && this.interactivityOptions.isDrillDownEnabled();
    }
    ...
    _getDefaultCalculationWindowDefinition() {
        return new CustomItemWindowDefinition();
    }
    getInfo() {
