...
*/
import { _setShowMessageFunc, appendStaticContextToRootViewModel, useKoIntegration } from '@devexpress/analytics-core/analytics-internal';
import { addToBindingsCache } from '@devexpress/analytics-core/analytics-widgets-internal';
import { EventsStrategy } from 'devextreme/core/events_strategy';
import { Options } from 'devextreme/core/options';
import dxPopup from 'devextreme/ui/popup';
import * as $ from 'jquery';
import * as ko from 'knockout';
import { $unwrap, createJQueryCallbacks, createJQueryDeferred, deepExtend, extend, getHeight } from '../data/_jquery-helpers';
import { groupByProperty, type } from '../data/_utils';
import { getLocalizationById } from '../data/localization/_default';
import { Dashboard } from '../model/dashboard';
import { safeComputed, subscribeArrayChange } from '../model/internal/_knockout-utils';
...
import { getBaseColorScheme } from '../viewer-parts/_dx-devextreme-themes-integration';
import { LegacySettings } from '../viewer-parts/legacy-settings';
import { createStylesElement } from '../viewer-parts/viewer/_cssHelper';
import { DataSourceBrowser } from './_data-source-browser';
import { CancellationToken, combineOptions, requestParamsValidator, toCancelableSequenceAction } from './_helpers';
import { DesignerDataServiceClient, ViewerDataServiceClient } from './_service-client';
import { defaultExtensions, designerExtensions, extensionNameMap } from './control-options';
...
import { getOptionNameByEvent } from './internal/_options-manager';
import { UpdateController } from './internal/_update-controller';
import { NotificationController } from './notification-controller/notificator';
import { AjaxRemoteService, FetchRemoteService } from './remote-service';
import { ResourceManager } from './resource-manager';
import { ViewerApi } from './viewer/_viewer-api';
import { ViewerItemAdaptersManager } from './viewer/adapters/_item-viewer-adapter-factory';
export class DashboardControl {
    constructor(element, options = {}) {
        this._repaintRequest = createJQueryCallbacks();
        ...
        this._workingModeChangeDeferred = createJQueryDeferred();
        useKoIntegration();
        createStylesElement(element, options.nonce);
        ...
        this._options = new Options(this._controlOptions, this._defaultControlOptions, this._getOptionsByReference(), this._getDeprecatedOptions());
        ...
        if (options.extensions) {
            for (let extensionName in options.extensions) {
                if (extensionNameMap.hasOwnProperty(extensionName)) {
                    const newName = extensionNameMap[extensionName];
        ...
        LegacySettings._useCardLegacyLayout = this.option('useCardLegacyLayout');
        ...
        this._dashboardContext = {
            beforeApplyViewerItemOptions: createJQueryCallbacks(),
            viewerItemCreated: createJQueryCallbacks(),
            viewerItemDispose: createJQueryCallbacks(),
            addContextToolbarItems: createJQueryCallbacks(),
            viewerItemCreator: {},
        ...
        for (let name in defaultExtensions) {
            extensionsToRegister[name] = defaultExtensions[name];
        }
        if (this.allowSwitchToDesigner) {
            for (let name in designerExtensions) {
                extensionsToRegister[name] = designerExtensions[name];
            }
        }
        _setShowMessageFunc((msg, type, displayTime, debugInfo) => { });
        this._dockingLayoutAdapter = new DockingLayoutAdapter(this);
        this._dockingLayoutAdapter.start();
        this._raiseEvent('initializing', { component: this });
        this._viewerApi = new ViewerApi(this);
        ...
        if (sequenceActions) {
            ...
            $.when(...actions.map(a => a(previousOptions, cancellationToken)))
                ...
                if (cancellationToken.canceled)
                    ...
                    DashboardControl._recursiveAsyncEval(sequence, cancellationToken, combineOptions(previousOptions, options), currentIndex + 1)
                        .done((opts) => {
                        if (cancellationToken.canceled)
                            def.reject();
                        else
    ...
    _raiseEvent(eventName, eventArgs) {
        ...
        var optionName = getOptionNameByEvent(eventName);
        if (optionName) {
    ...
    _getCebabCaseExtensionName(extensionName) {
        const index = Object.keys(extensionNameMap).map(k => extensionNameMap[k]).indexOf(extensionName);
        return index === -1 ? extensionName : Object.keys(extensionNameMap)[index];
    }
    _getCamelCaseExtensionName(extensionName) {
        return extensionNameMap[extensionName] || extensionName;
    }
    ...
    _updateRemoteServiceOptions(remoteServiceOptionsKey) {
        ...
        const remoteServiceType = remoteServiceOptionsKey === 'ajaxRemoteService' ? AjaxRemoteService : FetchRemoteService;
    ...
    _processExtensionsOptions(extensionsOptions) {
        Object.keys(extensionsOptions).forEach(key => {
            ...
            if (extensionOptions === false) {
                ...
                if (!extension) {
                    const extensionsFactory = Object.assign(Object.assign({}, defaultExtensions), designerExtensions);
                    const extensionCreator = extensionsFactory[this._getCebabCaseExtensionName(extensionName)];
                    type.isFunction(extensionCreator) && this.registerExtension(extensionCreator(this, extensionOptions));
                }
                else {
    ...
    get _colorSchemeCss() {
        return getBaseColorScheme() === 'light' ? 'dx-color-scheme-light' : 'dx-color-scheme-dark';
    }
    repaint() {
        this._repaintRequest.fire();
    }
    render() {
        ResourceManager._embedBundledResourcesCore(this._element);
    ...
    initializeDashboard(id, dashboardJson, initialState) {
        ...
        var dashboard = new Dashboard(dashboardJson);
        if (initialState) {
        ...
        let e = {
            ...
            dashboard: dashboard,
            dashboardId: id,
            ready: createJQueryDeferred().resolve().promise()
        };
        this._raiseEvent('dashboardInitializing', e);
        if (dashboard.disposed)
            return;
        e.ready.done(() => {
            dashboard.items().forEach(item => item._useNeutralFilterMode(this.option('useNeutralFilterMode')));
            this._perDashboardDisposables.push(subscribeArrayChange(dashboard.items, {
            ...
            this._dataSourceBrowser = new DataSourceBrowser(dashboard.dataSources, this.isDesignMode, dashboard.parameters, this._serviceClient, this._isLoading);
            this._dashboardContext.viewerItemsManager = new ViewerItemAdaptersManager();
            this._perDashboardDisposables.push(this._dashboardContext.viewerItemsManager);
            this._updateHubPrivate = new DashboardUpdateHub(dashboard, this.option('dataRequestOptions'), {
    ...
    requestDashboardList() {
        ...
        if (urls) {
            return this.remoteService.getFromServer(urls.GetDashboardsAction, null, { cache: false })
                .fail(() => {
                this.notificationController.showError(getLocalizationById('DashboardWebStringId.Notification.AttemptToLoadDashboards'));
            });
        }
        return createJQueryDeferred().reject().promise();
    }
    ...
    _loadDashboardCore(dashboardId) {
        let isValidRequest = requestParamsValidator.isValid({ dashboardId });
        ...
        if (urls && isValidRequest) {
            ...
            this.notificationController.showState(getLocalizationById('DashboardWebStringId.Notification.DashboardLoading'));
            return this.remoteService.getFromServer(urls.DashboardAction + '/' + encodeURIComponent(dashboardId), null, { cache: false })
                ...
                this.notificationController.showError(getLocalizationById('DashboardWebStringId.Notification.AttemptToLoadDashboard') + dashboardId, val);
            });
        }
        this._dashboardLoadingDeferred && this._dashboardLoadingDeferred.reject();
        this.notificationController.showError(getLocalizationById('DashboardWebStringId.Notification.AttemptToLoadDashboard') + String(dashboardId));
        return createJQueryDeferred().reject().promise();
    }
    unloadDashboard() {
    ...
    _switchToViewer() {
        var actions = this.extensions
            ...
            .map(extension => toCancelableSequenceAction(extension.designerToViewerAction))
            .sort((a, b) => a.orderNo - b.orderNo);
        let groups = groupByProperty(actions, 'orderNo');
        this._workingModeChangeDeferred.reject();
        this._workingModeChangeDeferred = createJQueryDeferred();
        return DashboardControl._recursiveAsyncEval(groups, new CancellationToken(this._workingModeChangeDeferred), { surfaceLeft: this._surfaceLeft(), surfaceTop: this._surfaceTop() })
            .then(options => {
    ...
    _switchToDesigner() {
        ...
        var actions = this.extensions
            ...
            .map(extension => toCancelableSequenceAction(extension.viewerToDesignerAction))
            .sort((a, b) => a.orderNo - b.orderNo);
        let groups = groupByProperty(actions, 'orderNo');
        this._workingModeChangeDeferred.reject();
        this._workingModeChangeDeferred = createJQueryDeferred();
    ...
    registerIcon(icon) {
        ResourceManager.registerIcon(icon);
    }
        ...
        extensionNames.forEach(extensionName => {
            ...
            if (!!extension) {
                ...
                if (customItemExtension.metaData) {
                    delete itemTypesMap[extension.name];
                }
    ...
    subscribeExtensionsChanged(handlers) {
        return subscribeArrayChange(this._extensions, handlers);
    }
    dispose() {
        this._unloadDashboardCore();
        ...
        ResourceManager._removeEmbeddedResourcesCore(this._element);
    }
    _renderDashboardItem(element, dashboardItemName, isStandalone = true) {
        ...
        this._applyBindings(element, {
            ...
            sizeController: new SingleItemSizeController(element, this._repaintRequest),
        ...
        if (extensionOptions !== false) {
            for (let extensionName in extensions) {
                ...
                if (options !== false) {
                    ...
                    this._controlOptions.extensions[camelName] = deepExtend({}, optionsManager && optionsManager.getInitialOptions(), options);
    ...
    _initializeServiceClient(dashboardContainer) {
        var serviceClient = this.isDesignMode() ?
            !this._validateWorkingMode() && new DesignerDataServiceClient(dashboardContainer, this.notificationController, this._endpointCollection.dataServiceUrls, this.remoteService) || null :
            new ViewerDataServiceClient(dashboardContainer, this.notificationController, this._endpointCollection.dataServiceUrls, this.remoteService);
    ...
    _addExtension(extension) {
        ...
        if (customItemExtension.metaData) {
            itemTypesMap[extension.name] = deepExtend({
                type: CustomItem,
    ...
    _applyBindings(element, additionalProperties) {
        var viewModel = Object.assign({ getWidgetContainer: this.getWidgetContainer, surfaceLeft: this._surfaceLeft, surfaceTop: this._surfaceTop, colorSchemeCss: this._colorSchemeCss, classList: () => this._classList, isDashboardLoaded: ko.computed(() => !!this.dashboard()), layoutTemplate: safeComputed({ info: this._layoutInfo }, ({ info }) => info.template), externalTemplates: this._externalTemplates, emptyControlTemplates: this._emptyControlTemplates, notificationController: this.notificationController, isLoading: this._isLoading, getLocalizationById: getLocalizationById, extend: extend, $unwrap: $unwrap, $: $, ko: ko }, additionalProperties);
    ...
    _createRemoteService(option) {
        if (type.isDefined(option.ajaxRemoteService) && type.isDefined(option.fetchRemoteService)) {
        ...
        if (type.isDefined(option.ajaxRemoteService)) {
            return new AjaxRemoteService(option.ajaxRemoteService);
        }
        return new FetchRemoteService(option.fetchRemoteService);
    }
}
...
addToBindingsCache('template: { name: \'dx-dashboard-control\' }', function () {
    return {
        'template': function () {
...
addToBindingsCache('dxControlsDescendantBindings: true', function () {
    return {
