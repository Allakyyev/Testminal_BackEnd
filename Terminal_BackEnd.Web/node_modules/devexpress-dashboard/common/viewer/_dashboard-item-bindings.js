...
import { getItemIconName, getItemTitle } from '../../model/internal/_dashboard-item_helper';
import { subscribeAndPerform } from '../../model/internal/_knockout-utils';
import { createElementSizeUpdater, createItemSizeUpdater, setElementSize } from './_element-size-utils';
export let DashboardItemHeaderHeight = 34;
function createViewerItemAdapter(element, dashboardItem, dashboardContext, localContext, sizeController) {
    ...
    let beforeRender = (viewerItem) => {
        ...
        _disposables.push(createItemSizeUpdater(viewerItem, sizeController));
...
function renderItemViewer(element, dashboardItem, dashboardContext, localContext, sizeController) {
    ...
    if (localContext && localContext.visualMode) {
        subscription = subscribeAndPerform(localContext.visualMode, _ => {
    ...
    if (dashboardItem._uiState() === 'loading') {
        ...
        return {
            template: {
                ...
                data: {
                    icon: getItemIconName(dashboardItem),
                    title: getItemTitle(dashboardItem),
    ...
    init: function (element, valueAccessor) {
        ...
        var itemViewerAdapter = params.dashboardContext.viewerItemsManager.create(params.dashboardItem, element, params.dashboardContext, params.localContext, viewerItem => {
            _disposables.push(ko.computed(() => {
                params.headerHeight(params.dashboardItem.showCaption() ? DashboardItemHeaderHeight : 0);
            }));
            setElementSize(element, sizeController);
            _disposables.push(createItemSizeUpdater(viewerItem, params.sizeController));
        });
        itemViewerAdapter.initialize();
        _disposables.push(itemViewerAdapter);
        ko.utils.domNodeDisposal.addDisposeCallback(element, function () {
...
export class DashboardUIItemStateController extends Disposable {
    constructor(_element, _dashboardItem, _sizeController, _bindingContext, _renderDashboardItem, _additionalClasses = []) {
        ...
        this._renderDashboardItemState = () => {
            ...
            if (this._dashboardItem._uiState() === 'live') {
                ...
                this._perUiStateSubscriptions.push(createElementSizeUpdater(this._element, this._sizeController));
                ko.applyBindingsToNode(this._element, { template: template }, this._bindingContext);
            }
        };
    }
    render() {
        if (this._dashboardItem.disposed)
            return;
        this._renderDashboardItemState();
        this.addDisposable(this._dashboardItem._uiState.subscribe(() => this._renderDashboardItemState()));
    }
    dispose() {
        this._perUiStateSubscriptions.forEach(d => d.dispose());
        this._perUiStateSubscriptions = [];
