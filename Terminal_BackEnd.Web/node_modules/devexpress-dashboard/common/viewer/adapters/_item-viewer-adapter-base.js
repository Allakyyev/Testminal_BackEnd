...
*/
import { closest, wrapPublicElement } from '../../../data/_jquery-helpers';
import { DisposableObject } from '../../../model/disposable-object';
import { fromUtcDateToString } from '../../../model/internal/_date-utils';
import { CustomItem } from '../../../model/items/custom-item/custom-item';
import { defaultViewerItemFactory } from '../../../viewer-parts/_viewer-item-factory';
import { getControlContainer } from '../../../viewer-parts/viewer-items/_base-item';
import { CustomItemViewer } from '../../custom-viewer-item/custom-viewer-item';
export class ItemViewerAdapterBase extends DisposableObject {
    ...
    createDashboardViewerItem(element, content, dashboardItem) {
        element.innerHTML = '';
        content.parentContainer = closest(element, '.dx-dashboard-container');
        content.controlContainer = getControlContainer(element);
        ...
        var viewerItem;
        if (this.dashboardItem instanceof CustomItem) {
            if (visualMode === 'caption') {
                viewerItem = new CustomItemViewer(this.dashboardItem, wrapPublicElement(element), content);
            }
            else {
                var viewerItemCreator = this.context.viewerItemCreator[this.dashboardItem.customItemType()];
                if (!!viewerItemCreator) {
                    viewerItem = viewerItemCreator(this.dashboardItem, wrapPublicElement(element), content);
                }
            }
        }
        if (!viewerItem) {
            var viewerItemFactory = (this.localContext && this.localContext.itemFactory) || defaultViewerItemFactory;
        ...
        viewerItem.dateToString = fromUtcDateToString;
        viewerItem.render();
        return viewerItem;
    }
