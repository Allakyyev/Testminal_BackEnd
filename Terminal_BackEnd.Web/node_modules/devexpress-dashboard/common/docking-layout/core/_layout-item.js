...
import { DashboardViewerConstants } from '../../../viewer-parts/viewer/_dashboard-viewer-constants';
export function _syncLayoutHelper(sourceArray, destArray, addHandler) {
    if (sourceArray) {
...
export function setHoverLocation(hoverLayoutItem, location = null, dropBehavior = 'InsertBesideGroup') {
    if (hoverLayoutItem) {
...
export let SplitterSize = 6;
export const MinWeight = 0.00001;
export class LayoutItem extends Disposable {
    constructor(viewModel, parent) {
        ...
        this.setConstraints = (constraints) => {
            ...
            this._constraints.min.width(correctSize(constraints.min.width, SplitterSize));
            this._constraints.min.height(correctSize(constraints.min.height, SplitterSize));
            this._constraints.max.width(correctSize(constraints.max.width, SplitterSize));
            this._constraints.max.height(correctSize(constraints.max.height, SplitterSize));
        };
        ...
        this.repaintCallbacks = createJQueryCallbacks();
        var self = this;
        this.addDisposable(this.itemStyle = ko.computed(() => {
            var result = '';
        ...
        this._constraints = {
            min: {
                width: ko.observable(viewModel.hasItem() ? DashboardViewerConstants.minPaneWidth : 0),
                height: ko.observable(viewModel.hasItem() ? DashboardViewerConstants.minPaneHeight : 0)
            },
            max: {
        ...
        this.addDisposable(this.visible = ko.computed({
            read: () => {
        ...
        this.addDisposable(this.minWidth = ko.computed(() => {
        ...
        this.addDisposable(this.minHeight = ko.computed(() => {
        ...
        this.addDisposable(this.maxWidth = ko.computed(() => {
            return Math.min(Number.MAX_VALUE, this._constraints.max.width());
        }));
        this.addDisposable(this.maxHeight = ko.computed(() => {
        ...
        this.addDisposable(this.width = ko.computed({
            read: () => {
        ...
        this.addDisposable(this.height = ko.computed({
            read: () => {
        ...
        this.addDisposable(this.contentWidth = ko.computed(() => this.width() - this.horizontalPaddings()));
        this.addDisposable(this.contentHeight = ko.computed(() => this.height() - this.verticalPaddings()));
        this.addDisposable(this.containerWidth = ko.computed(() => this.width() - SplitterSize));
        this.addDisposable(this.containerHeight = ko.computed(() => this.height() - SplitterSize));
        this.addDisposable(this.areChildrenSelected = ko.pureComputed(() => {
        ...
        this.addDisposable(_syncLayoutHelper(viewModel.visibleItems, this.items, model => new LayoutItem(model._createViewModel(), self)));
        ...
        this.addDisposable(this.height.subscribe(() => fireRepaintCallback()));
        this.addDisposable(this.width.subscribe(() => fireRepaintCallback()));
        this.addDisposable(this.isLayoutReady.subscribe(() => fireRepaintCallback()));
        fireRepaintCallback();
        var placeholder = this.viewModel.getPlaceholder && this.viewModel.getPlaceholder();
        if (placeholder) {
            this.addDisposable(placeholder);
            this.placeholderItem = new LayoutItem(placeholder._createViewModel(), this);
        }
        this.addDisposable(this.visible, this.minWidth, this.minHeight, this.maxWidth, this.maxHeight, this.width, this.height, this.areChildrenSelected);
    }
...
ko.components.register('dx-layout-item', {
    viewModel: {
        createViewModel: (params, componentInfo) => {
            const layoutItem = ko.computed(() => ko.unwrap(params.layoutItem));
