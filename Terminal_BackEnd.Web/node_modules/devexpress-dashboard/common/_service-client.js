...
import { deepExtend, jqueryQueryParam } from '../data/_jquery-helpers';
import { getLocalizationById } from '../data/localization/_default';
import { DataDashboardItem } from '../model/items/data-dashboard-item';
import { _getParametersQuery } from '../model/parameters/parameter';
import { requestParamsValidator } from './_helpers';
export class ViewerDataServiceClient {
    constructor(_dashboardContainer, _errorHandler, _dataServiceUrls, _remoteService) {
        ...
        this.getColoringScheme = (itemName = '') => {
            throw new Error(getLocalizationById('DashboardWebStringId.Notification.MethodIsNotAllowedInViewerMode'));
        };
    ...
    getParameterValues(dataSourceId, dataSource, dataMember, valueMember, displayMember, sortOrder, sortByMember, parameterType) {
        return this._performRequest(this._dataServiceUrls.ParameterValuesAction, {
            ...
            .fail((request) => this._errorHandler.showError(getLocalizationById('DashboardWebStringId.Notification.AttemptToLoadParameterValues') + (dataSource && dataSource.name() || dataSourceId), request));
    }
    performExport(exportInfo, exportModels) {
        ...
        this._remoteService.performPostback(this._dataServiceUrls.PerformExportAction, args)
            .fail((response) => this._errorHandler.showError(getLocalizationById('DashboardWebStringId.Notification.AttemptToPerformExport'), response));
        return;
    }
    ...
    _getItemDataQuery(item, isModeAllowsToReduceData) {
        let dataQueryParams = deepExtend({}, item._getDataQueryParams());
        let isDataItem = item instanceof DataDashboardItem;
    ...
    _queryParamsToQueryString(dataQueryParams) {
        ...
        return Object.keys(urlQuery).length === 0 ? '' : jqueryQueryParam(urlQuery);
    }
    _createCommonArgs() {
        return {
            dashboardId: this._dashboardContainer.id,
            parameters: _getParametersQuery(this._dashboardContainer.dashboard.parameters())
        };
    }
    ...
    _ensureClientServerContract(requestParams) {
        if (requestParamsValidator.isValid(requestParams)) {
...
export class DesignerDataServiceClient extends ViewerDataServiceClient {
    constructor(_dashboardContainer, _errorHandler, _dataServiceUrls, _remoteService) {
        ...
        this.getColoringScheme = (itemName = '') => {
            return this._performRequest(this._dataServiceUrls.GetColoringSchemeAction, {
                ...
                .fail(request => { this._errorHandler.showError(getLocalizationById('DashboardWebStringId.Notification.AttemptToGetColorScheme'), request); });
        };
        this.getDashboardPalette = () => {
            const requestArgs = {
                dashboardId: this._dashboardContainer.id
    ...
    convertItem(item, destinationItemTypeName) {
        return this._performRequest(this._dataServiceUrls.ConvertItemAction, {
            ...
            .fail((request) => { this._errorHandler.showError(getLocalizationById('DashboardWebStringId.Notification.AttemptToConvertItemToType') + destinationItemTypeName, request); });
    }
    getDimensionUniqueValues(dataSource, dataMember, dimension) {
        return this._performRequest(this._dataServiceUrls.DimensionUniqueValuesAction, {
            dataSourceId: dataSource.componentName(),
            dimension: new ModelSerializer({ useRefs: false }).serialize(dimension),
            dataMember: dataMember,
        })
            .fail((request) => { this._errorHandler.showError(getLocalizationById('DashboardWebStringId.Notification.AttemptToLoadDimensionUniqueValues') + dataSource.name(), request); });
    }
    ...
    getFieldList(dataSource, dataMember, fieldPath) {
        return this._performRequest(this._dataServiceUrls.FieldListAction, {
            ...
            .fail(request => { this._errorHandler.showError(getLocalizationById('DashboardWebStringId.Notification.AttemptToLoadFieldListForDataSource') + dataSource.name(), request); });
    }
    _createCommonArgs() {
        return {
            dashboardId: this._dashboardContainer.id,
