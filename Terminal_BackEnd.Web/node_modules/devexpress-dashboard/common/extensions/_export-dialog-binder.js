...
*/
import { getOuterHeight, getOuterWidth, jqueryOffset } from '../../data/_jquery-helpers';
import { GridItem } from '../../model';
import { DisposableObject } from '../../model/disposable-object';
import { toStringArray } from '../../model/internal/_date-utils';
...
export class ExportDialogBinder {
    ...
    _getClientSize(container, mode) {
        if (container) {
            return { width: getOuterWidth(container), height: getOuterHeight(container) };
        }
        else {
    ...
    _getActualComponentName(item) {
        return item instanceof TabContainerItem ? item._activeTabPage().componentName() : item.componentName();
    }
    _getExportHolderItem(item) {
        if (item instanceof TabContainerItem) {
            return this._options.dashboard()._getDisplayDashboardItem(item._activeTabPage());
        }
        return item;
    }
    _getInfo(items, titleHeight, mode, format) {
        var container = this._options.getContainer().querySelector('.dx-dashboard-viewer'), containerPosition = container ? jqueryOffset(container) : { left: 0, top: 0 }, clientSize = this._getClientSize(container, mode);
        let elementsList = items
            .map(item => {
            let holderItem = this._getExportHolderItem(item);
            let isTabContainer = item instanceof TabContainerItem;
        ...
        if (exportInfo.Mode === 'EntireDashboard') {
            if (!isGroupExport) {
                if (exportInfo.Format === 'Excel' && excelOptions && excelOptions.DashboardStatePosition === 'Below') {
                    ...
                    if (item instanceof DataDashboardItem && dashboard._masterFilterItems().indexOf(item) !== -1) {
    ...
    exportDashboardTo(format, clientOptions, customFileName) {
        ...
        var fileName = customFileName ? customFileName : (clientOptions == undefined || clientOptions.fileName == undefined) ? 'Export' : clientOptions.fileName, dashboard = this._options.dashboard(), items = dashboard
            ...
            .filter(node => !(node.item instanceof DashboardTabPage))
            .map(node => node.item);
        ...
        this._exportTo(items, {
            ...
            format: format,
            clientState: this._getInfo(items, this._options.dashboard().title.visible() ? titleHeight : 0, 'EntireDashboard', format),
            fileName: fileName
        }, exportOptions);
    }
    exportDashboardItemTo(itemComponentName, format, clientOptions, customFileName) {
        var fileName = customFileName ? customFileName : (clientOptions == undefined || clientOptions.fileName == undefined) ? 'Export' : clientOptions.fileName, dashboard = this._options.dashboard(), exportItem = dashboard.findItem(itemComponentName), isGroup = exportItem instanceof GroupItem || exportItem instanceof DashboardTabPage;
    ...
    get exportDialog() {
        if (!this._exportDialog) {
            this._exportDialog = new exportDialog({
    ...
    _getExportItemType(item) {
        return item ? DashboardItem._getCommonItemType(item.itemType()) : undefined;
    }
    ...
    _exportTo(items, modeInfo, documentOptions) {
        ...
        var model = items.map(item => {
            ...
            return {
                ...
                drillDownFormattableValues: holderItem instanceof DataDashboardItem ? toStringArray(holderItem._getDisplayDrillDownValues()) : undefined,
                filterFormattableValues: toStringArray(this._getFilterFormattableValues(dashboard, exportInfo.GroupName, holderItem, exportInfo)),
                selectedValues: toStringArray(holderItem._getExportingSelection())
            };
        });
    ...
    _getExportDataQueryParams(dashboardItem, modeInfo) {
        ...
        if (dashboardItem instanceof GridItem && dashboardItem.columnFilterOptions.updateTotals()) {
...
export class ExportInfoManager extends DisposableObject {
    constructor() {
        super(...arguments);
