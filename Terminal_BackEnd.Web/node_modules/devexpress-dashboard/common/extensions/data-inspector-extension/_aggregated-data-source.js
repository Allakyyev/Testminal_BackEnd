...
import { deepStrictEquals, distinct, type } from '../../../data/_utils';
import { dateFilterElement } from '../../../viewer-parts/viewer-items/filter-items/_date-filter-element';
import { getMeasureColumns, getSortedAxes } from './_inspected-data-colum-generator';
...
function _createMeasureColumnInfo(itemData, measure, isSparklineMeasure) {
    ...
    var getSparklineCellText = (defaultAxisPoint) => {
        let argumentValueSeparator = getLocalizationById('DashboardStringId.SparklineArgumentValueSeparator');
        let valuesSeparator = getLocalizationById('DashboardStringId.SparklineValuesSeparator');
        return itemData
            .getAxis('Sparkline')
            .getPoints()
            .map(sparklinePoint => {
    ...
    return {
        ...
        getGridColumn: () => {
            ...
            switch (measure.id) {
                case dateFilterElement.MinMeasureId:
                    caption += ' ' + getLocalizationById('DashboardStringId.StartDate');
                    break;
                case dateFilterElement.MaxMeasureId:
                    caption += ' ' + getLocalizationById('DashboardStringId.EndDate');
                    break;
            }
            return {
                dataField: dataField,
...
function _createColumns(itemData, axes, args) {
    return axes
        .reduce((acc, axis) => {
        let distinctDimensions = distinct(itemData.getDimensions(axis), (dim1, dim2) => dataInspectorItemDataDimensionComparer(dim1, dim2, itemData));
        return acc.concat(distinctDimensions.map(dimension => _createDimensionColumnInfo(dimension, axis)));
    }, [])
        .concat(distinct(getMeasureColumns(itemData), (m1, m2) => dataInspectorItemDataMeasureComparer(m1, m2, itemData, args.addSparklineTotal, args.sparklineMeasures))
        .reduce((acc, measure) => {
...
function _createRows(itemData, axes) {
    if (axes.length) {
        return itemData
            ...
            if (axes.length > 1) {
                acc = acc.concat(itemData
                    .getAxis(axes[1])
                    .getAvaliableLeafPoints()
                    .map(secondaryAxisPoint => new itemDataTuple([primaryAxisPoint, secondaryAxisPoint])));
            }
            else {
                acc.push(new itemDataTuple([primaryAxisPoint]));
            }
            return acc;
        }, []);
    }
        ...
        return [new itemDataTuple(axisPoints)];
    }
}
export function generateAggregatedSource(itemData, args) {
    if (itemData && !itemData.isEmpty()) {
        let axes = getSortedAxes(itemData, true);
...
export function dataInspectorItemDataDimensionComparer(x, y, data) {
    return x.name === y.name
        && x.dataMember === y.dataMember
        && x.dateTimeGroupInterval === y.dateTimeGroupInterval
        && x.textGroupInterval === y.textGroupInterval
        && deepStrictEquals(data.getDimensionFormat(x.id), data.getDimensionFormat(y.id));
}
export function dataInspectorItemDataMeasureComparer(x, y, data, addSparklineTotal, sparklineMeasures) {
    return x.name === y.name
        && x.dataMember == y.dataMember
        && (addSparklineTotal ||
            !sparklineMeasures || ((sparklineMeasures.indexOf(x.id) !== -1) === (sparklineMeasures.indexOf(y.id) !== -1)))
        && ((type.isDefined(data.getMeasureExpression(x.id)) || type.isDefined(data.getMeasureExpression(y.id))) ?
            (data.getMeasureExpression(x.id) === data.getMeasureExpression(y.id)) : (x.summaryType === y.summaryType))
        && deepStrictEquals(data.getMeasureCalculation(x.id), data.getMeasureCalculation(y.id))
        && deepStrictEquals(data.getMeasureWindowDefinition(x.id), data.getMeasureWindowDefinition(y.id))
        && deepStrictEquals(data.getMeasureFormat(x.id), data.getMeasureFormat(y.id));
}
