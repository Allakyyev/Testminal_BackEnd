...
import { DashboardDragItemInfo } from '../../common/docking-layout/drag-and-drop/_drag-item-info';
import { KeyCodes } from '../../common/internal/_interfaces';
import { subscribeWithPrev } from '../../model/index.internal';
...
export class ToolboxViewModel {
    constructor(menuVisible, _defaultMenuItemData, _menuItems, _toolboxGroups, _toolbarGroups, _layoutDragController) {
        ...
        this.designerPanelLeft = ko.observable(menuVisible ? 0 : -toolboxConstants.leftPanelWidth);
    ...
    initDragEvents(itemType) {
        ...
        return {
            onInitialize: (args) => {
                ...
                subscription = subscribeWithPrev(this._layoutDragController, (prevValue, newValue) => {
    ...
    processKeyEvent(keyEventType, eventArgs) {
        if (keyEventType === 'keyup') {
            ...
            if (eventArgs.keyCode === KeyCodes.Esc && this.menuVisible()) {
                this.closeMenu();
                return true;
            }
    ...
    hideDesignerPanel() {
        this.designerPanelLeft(-toolboxConstants.leftPanelWidth);
    }
}
class ToolboxItemDragItemInfo extends DashboardDragItemInfo {
    constructor(_itemType) {
        super();
        this._itemType = _itemType;
    }
    canDrop(layoutItem) {
        return layoutItem.canAttach(getItemJson(this._itemType));
    }
    drop(dragOverState) {
        dragOverState.targetItem.create(getItemJson(this._itemType), dragOverState.hoverLocation, dragOverState.targetItemBehavior);
    }
    dragStart() { }
    dragEnd() { }
}
