...
export class CreateDashboardExtension {
    constructor(dashboardControl) {
        ...
        this._createNewDashboard = (dataSources, dashboardName) => {
            ...
            let createDashboardCallback = () => {
                var dashboardPropotype = new Dashboard({});
                dashboardPropotype.title.text(dashboardName);
                dataSources.forEach(dataSource => {
                    var newDataSource = Dashboard._createDataSource(new ModelSerializer({ useRefs: false }).serialize(dataSource), new ModelSerializer());
        ...
        this._newDashboardMenuItem = new DashboardMenuItem(nameAlias, 'DashboardWebStringId.DashboardMenuNew', 105, 78, () => clickHandler());
    }
    start() {
        var extension = this.dashboardControl.findExtension('toolbox');
        if (extension) {
    ...
    performCreateDashboard(dashboardName, dashboardJson) {
        this.dashboardControl.notificationController.showState(getLocalizationById('DashboardWebStringId.Notification.DashboardLoading'));
        return this.dashboardControl.remoteService.postToServer(this.dashboardControl._endpointCollection.dashboardUrls.GetDashboardsAction, { name: dashboardName, dashboard: dashboardJson })
            ...
            this.dashboardControl.notificationController.showError(getLocalizationById('DashboardWebStringId.Notification.NewDashboardCannotBeCreated'), request);
        });
    }
}
designerExtensions[nameAlias] = (dashboardControl, options) => new CreateDashboardExtension(dashboardControl);
...
class CreateNewDashboardViewModel {
    constructor(accessibleDataSourcesExtension, _newDashboardCallback) {
        ...
        this.dashboardName(getLocalizationById('DashboardWebStringId.NewDashboard'));
