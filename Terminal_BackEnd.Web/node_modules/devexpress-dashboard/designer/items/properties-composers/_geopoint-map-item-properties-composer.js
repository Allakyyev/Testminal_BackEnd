...
export class GeoPointMapItemPropertiesComposer extends DashboardItemPropertiesComposer {
    constructor(customizeHandler) {
        super(customizeHandler);
    }
    _composeTabsCore(model, args) {
        var tabModel = SharedComposers.getCommonMapWrapper(model, args.propertiesController, [
            lockNavigation,
            enableClustering,
            SharedComposers.getShapeTitleSerializationInfo(model)
        ]);
        return [
            new AccordionTab(KnownTabs.Common, 'DashboardWebStringId.AccordionTab.Common', tabModel)
        ];
    }
}
