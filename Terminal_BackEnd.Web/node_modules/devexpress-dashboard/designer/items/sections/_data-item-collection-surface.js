...
export class DataItemCollectionSurface {
    constructor(itemSurface, sectionInfo, extendTabsHandler, warning) {
        ...
        this.addClick = () => {
            var dataLink = new DataItemLink(this.itemSurface.dashboardItem);
            this.newItemSample(dataLink);
            var surface = new DataItemSurface(dataLink, this.sectionInfo.bindingProperty, this.itemSurface.dashboardItem, this.itemSurface._dataSourceBrowser, this.itemSurface.propertiesController, true, null, this.extendTabsHandler);
        ...
        this.selectDataItem = (data) => {
            var surface = new DataItemSurface(data, this.sectionInfo.bindingProperty, this.itemSurface.dashboardItem, this.itemSurface._dataSourceBrowser, this.itemSurface.propertiesController, true, null, this.extendTabsHandler);
        ...
        this.groups = ko.computed(() => {
            ...
            this.dataItems().forEach(dataItemLink => {
                if (dataItemLink.dataItem() instanceof Dimension && IsOlapHierarchyField(dataItemLink.dataItem())) {
    ...
    _removeDataItem(link) {
        ...
        if (dataItem instanceof Dimension && IsOlapHierarchyField(dataItem)) {
            for (var i = this.dataItems().length - 1; i >= 0; i--) {
                ...
                if (groupItem instanceof Dimension && groupItem.groupIndex() == dataItem.groupIndex()) {
    ...
    isOlap() {
        ...
        return DataField.isOlap(filledItem.dataItem().dataMember());
    }
    errorFactory(link) {
