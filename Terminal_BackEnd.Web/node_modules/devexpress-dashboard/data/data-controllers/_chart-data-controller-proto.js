...
import { parsePointLabelContentType, PointLabelContentType } from '../../model/enums';
import { findClosestItemIndex } from '../../model/internal/_array-utils';
import { tryConvertToDateTime } from '../../model/internal/_date-utils';
import { RenderHelper } from '../../viewer-parts/widgets/_render-helper';
import { chartHelper } from '../_chart-helper';
import { DashboardDataIdField } from '../_common';
import { convertToFormat, formatByFormatInfo, formatPercentValue, format as formatter, getAxisFormat } from '../_formatter';
import { localizationId } from '../_localization-ids';
import { localizer } from '../_localizer';
import { encodeHtml as encoder, getRGBColor, type } from '../_utils';
...
export class chartDataControllerProto extends chartDataControllerBase {
    constructor(options) {
        super(options);
    ...
    getDataSourceAndSeries(encodeHtml) {
        ...
        if (seriesInfoList.length > 0) {
            ...
            legendInfoList.forEach((legendInfo) => {
                ...
                let insertIndex = findClosestItemIndex(result.series, (series) => {
            ...
            this._argumentAxisPoints.forEach((argumentAxisPoint, pointIndex) => {
                ...
                seriesInfoList.forEach(seriesInfo => {
                    ...
                    seriesInfo.valueFields.forEach(valueField => {
                        ...
                        if (value === specialValues.errorValueGuid)
                            value = 0;
            ...
            seriesAxisPoints.forEach(seriesAxisPoint => {
                ...
                (_a = seriesTemplate.IndicatorViewModels) === null || _a === void 0 ? void 0 : _a.forEach(indicatorViewModel => {
                    ...
                    var info = generateSeriesInfo(pane, indicatorViewModel, paneIndex, seriesAxisPoint, {
                        ...
                        staticColor: getRGBColor(color.R, color.G, color.B, color.A)
                    }, true);
                    add(info);
                });
            });
    ...
    _getTooltipHtml(series, point, seriesFormats, encodeHtml, color) {
        ...
        if (color) {
            ...
            div.appendChild(RenderHelper.rectangleHtml(color, 10, 10, '5px'));
    ...
    _getTooltipSpanInternal(series, point, seriesFormats, encodeHtml) {
        ...
        const displayText = series.name + ': ';
        element.innerHTML = encodeHtml ? encoder(displayText) : displayText;
        switch (series.type) {
            case 'rangebar':
            case 'rangearea':
                element.innerHTML += that._formatValuesList([point.originalMinValue, point.originalValue], seriesFormats, encodeHtml);
                break;
            case 'bubble':
                element.innerHTML += that._formatValuesList([point.originalValue, point.size], seriesFormats, encodeHtml);
                break;
            case 'stock':
            case 'candlestick':
                ...
                break;
            default:
                element.innerHTML += that._formatValuesList([point.originalValue], seriesFormats, encodeHtml);
                break;
        }
        return element;
    }
    ...
    _createAxisXFormat() {
        if (this.viewModel.AxisX.Format && (this.viewModel.AxisX.Format.NumericFormat != null || this.viewModel.AxisX.Format.DateTimeFormat != null))
            return convertToFormat(this.viewModel.AxisX.Format);
    ...
    _getArgumentText(argumentValue, formatGetter) {
        if (this.isDiscreteArgument() || this.isQualitativeArgument()) {
            ...
            if (this.viewModel.Argument.AxisXDateTimeFormatSupported)
                return formatByFormatInfo(axisPoint.getValue(), formatGetter());
            return axisPoint ? this.getTitle(axisPoint, '\n') : argumentValue;
        }
        return formatByFormatInfo(argumentValue, formatGetter());
    }
    ...
    _getConditionalFormattingRangeLegendItems(rule) {
        ...
        for (let index = rangeConditionModel.Ranges.length - 1; index >= 0; index--) {
            ...
            if (rangeConditionModel.ValueType === 'Percent') {
                leftValueString = formatPercentValue(leftValue ? leftValue / 100 : maxPercentValue);
                rightValueString = formatPercentValue(rightValue / 100);
            }
            else {
                leftValueString = leftValue ? formatter(tryConvertToDateTime(leftValue), dataItemFormat) : positiveInfinitySymbol;
                rightValueString = rightValue ? formatter(tryConvertToDateTime(rightValue), dataItemFormat) : negativeInfinitySymbol;
            }
            let legendDescription = stringFormat(localizer.getString(localizationId.LegendItemFormatString), leftValueString, rightValueString);
    ...
    _createSeriesItem(seriesInfo, includeTags, encodeHtml) {
        var that = this, seriesItem = {
            ...
            if (type.isDefined(seriesInfo[propertyName]))
        ...
        seriesItem[DashboardDataIdField] = seriesInfo.dataId;
        ...
        else if (type.isDefined(seriesInfo.colorMeasureId)) {
        ...
        if (chartHelper.isSeriesColorSupported(seriesItem.type)) {
            var argumentRootAxisPoint = this.multiData.getAxis(itemDataAxisNames.chartArgumentAxis).getRootPoint();
        ...
        else if (type.isDefined(seriesInfo.staticColor)) {
            seriesItem.color = seriesInfo.staticColor;
        }
        if (chartHelper.isStackedAreaType(seriesItem.type)) {
    ...
    _customizePointLabelText(valueContainer, pointLabel, seriesInfo) {
        ...
        let hasArgument = (contentType & PointLabelContentType.Argument) > 0, hasSeriesName = (contentType & PointLabelContentType.SeriesName) > 0, hasValue = (contentType & PointLabelContentType.Value) > 0, hasPercent = (contentType & PointLabelContentType.Percent) > 0 && this._getPercentSupported(seriesInfo);
        let firstPart = '';
        let secondPart = '';
        let argument = hasArgument ? formatArgumentProc() : '';
        let series = hasSeriesName ? valueContainer.seriesName : '';
        let percent = hasPercent ? formatPercentValue(valueContainer.percent) : '';
        if (hasArgument) {
    ...
    _formatOpenHighLowCloseValues(values, formats, hasOpenValueField, encodeHtml, delimiter) {
        var result = [], delimiter = delimiter || ' ', formatsLength = formats ? formats.length : 0, i, formatIndex, valueNames = [
            localizer.getString(localizationId.OpenCaption),
            localizer.getString(localizationId.HighCaption),
            localizer.getString(localizationId.LowCaption),
            localizer.getString(localizationId.CloseCaption)
        ];
        if (values && (formatsLength > 0) && (formatsLength <= values.length)) {
            for (formatIndex = 0, i = hasOpenValueField ? 0 : 1; formatIndex < formatsLength; formatIndex++, i++) {
                ...
                span.innerHTML = valueNames[i] + ': ' + this._formatValue(values[i], formats[formatIndex], encodeHtml);
                result.push(span);
            }
        }
        return result;
    ...
    _formatValue(value, format, encodeHtml) {
        var text = formatter(value ? value : 0, format);
        return encodeHtml ? encoder(text) : text;
    }
    _isNumericDataType(type) {
        return type == 'Integer' || type == 'Float' || type == 'Double' || type == 'Decimal';
    }
    _convertContentType(typeModel) {
        return parsePointLabelContentType(typeModel);
    }
    _showPointMarker(seriesTemplate) {
        if (seriesTemplate.SeriesType === 'Point') {
            return true;
        }
        else if (allowedTypesForShowPointMarkers.indexOf(seriesTemplate.SeriesType) !== -1) {
    ...
    showPointLabels(pointLabelInfo) {
        return pointLabelInfo && pointLabelInfo.contentType != PointLabelContentType.None;
    }
    _getPointLabelInfo(pointLabelViewModel) {
        if (pointLabelViewModel) {
            return {
                showPointLabels: pointLabelViewModel.ShowPointLabels,
                rotationAngle: chartHelper.convertPointLabelRotationAngle(pointLabelViewModel.Orientation),
                position: chartHelper.convertPointLabelPosition(pointLabelViewModel.Position),
...
export const allowedTypesForShowPointMarkers = ['Line', 'Spline', 'StepLine', 'StackedLine', 'FullStackedLine', 'SplineArea', 'Area', 'StepArea', 'RangeArea'];
