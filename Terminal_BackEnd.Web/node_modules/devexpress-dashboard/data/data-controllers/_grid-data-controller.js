...
import { GridBarCalculator } from '../_grid-bar-calculator';
...
export class gridDataController extends dataControllerBase {
    constructor(options) {
        ...
        var pushColumn = (columnName, columnViewModel) => {
            this._columnRepository[columnName] = {
                ...
                barCalculator: columnViewModel.BarViewModel ? new GridBarCalculator(columnViewModel.BarViewModel.AlwaysShowZeroLevel) : null
            };
        ...
        $.each(that._axisColumnPoints, function (index, axisPoint) {
            ...
            $.each(selectionMembers, function (memberIndex, member) {
                ...
                if (point && values.length > memberIndex) {
                    if (!checkValuesAreEqual(point.getUniqueValue(), values[memberIndex], true)) {
                        fit = false;
                        return false;
                    }
                    resultPoint = point;
    ...
    getSelectedRowKeys(valuesSet) {
        var that = this, keys = [], selectionMembers = that._selectionMembers, checkAxisPoint = function (axisPoint, values) {
            ...
            for (var i = 0; i < values.length; i++) {
                ...
                if (!checkValuesAreEqual(point.getUniqueValue(), values[i], true))
                    return false;
            }
            return true;
        };
    ...
    _getStyleSettingsInfo(columnName, rowIndex) {
        ...
        return that._getStyleSettingsInfoCore(cellInfo, rules, that.viewModel.ColumnAxisName, itemDataAxisNames.defaultAxis);
    }
