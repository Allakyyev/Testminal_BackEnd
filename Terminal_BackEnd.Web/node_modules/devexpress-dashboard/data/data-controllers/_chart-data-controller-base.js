...
import { itemDataAxisHelper } from '../item-data/internal/_item-data-axis-helper';
...
import { toColor, type } from '../_utils';
...
export class chartDataControllerBase extends dataControllerBase {
    ...
    isQualitativeArgument() {
        return this._getArgumentAxisDimensions().length > 1 && !this.drillDownState[itemDataAxisNames.chartArgumentAxis];
    }
    isSingleArgument() {
        return this._getArgumentAxisDimensions().length == 1 || this.drillDownState[itemDataAxisNames.chartArgumentAxis] != null;
    }
    hasSeriesPoints() {
    ...
    _getStyleSettingsInfo(argumentAxisPoint, seriesAxisPoint, seriesMeasureId, chartElement) {
        if (this.cfModel) {
            ...
            return this._getStyleSettingsInfoCore(seriesInfo, rules, this.viewModel.ColumnAxisName, itemDataAxisNames.defaultAxis);
        }
    }
    _getStyleIndexes(rule, seriesInfo, points) {
        let isExpressionRule = rule.CalcByDataId ? false : true;
        let isDimensionRule = this.multiData.getDimensions(itemDataAxisNames.chartSeriesAxis).concat(this.multiData.getDimensions(itemDataAxisNames.chartArgumentAxis)).filter(dimension => dimension.id === rule.CalcByDataId).length > 0;
    ...
    findStylesForDimension(seriesPoint, argumentPoint, idDimensionCalculateBy, formatRuleMeasureId) {
        let isSeriesDimension = this.multiData.getDimensions(itemDataAxisNames.chartSeriesAxis).filter(dimension => dimension.id === idDimensionCalculateBy).length > 0, slicePoint = (isSeriesDimension ? seriesPoint : argumentPoint).getParentByDimensionId(idDimensionCalculateBy);
    ...
    getStyleIndexesOnArgumentAxis(seriesAxisPoint, cfMeasureId, isExpressionRule, isDimensionRule, idDimensionCalculateBy) {
        let helper = itemDataAxisHelper, styles = [], rootArgumentPoint = this.multiData.getAxis(itemDataAxisNames.chartArgumentAxis).getRootPoint();
        let targetSeriesPoint = seriesAxisPoint ? seriesAxisPoint : this.multiData.getAxis(itemDataAxisNames.chartArgumentAxis).getRootPoint();
    ...
    _getArgumentAxis() {
        return this.multiData ? this.multiData.getAxis(itemDataAxisNames.chartArgumentAxis) : undefined;
    }
    _getSeriesAxis() {
        return this.multiData ? this.multiData.getAxis(itemDataAxisNames.chartSeriesAxis) : undefined;
    }
