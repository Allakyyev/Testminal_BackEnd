...
*/
import Color from 'devextreme/color';
import browser from 'devextreme/core/utils/browser';
import dxTooltip from 'devextreme/ui/tooltip';
import * as $ from 'jquery';
import { accessJQueryData, wrapInner } from '../../data/_jquery-helpers';
import { toColor } from '../../data/_utils';
import { createCssClass, createCssPropertyWrapper } from '../viewer/_cssHelper';
import { layoutCardCssClassNames } from '../widgets/widgets-viewer/_css-class-names';
import { appearanceSettingsProvider } from './_appearance-settings-provider';
...
export class styleSettingsProvider {
    ...
    static _getRangeBackColorStyleSettings(styleSettings, condition) {
        ...
        leftColor = new Color(appearanceSettingsProvider.getColor(leftModel));
        rightColor = new Color(appearanceSettingsProvider.getColor(rightModel));
        return {
        ...
        if (!this.drawingLocked) {
            ...
            if (barCacheItems) {
                ...
                barCacheItems.forEach((barCacheItem, i) => {
                    var bar = barCacheItem.barDiv, normalizedValue = accessJQueryData(bar, this.DataAttributes.NormalizedValue), zeroPosition = accessJQueryData(bar, this.DataAttributes.ZeroPosition), allowNegativeAxis = accessJQueryData(bar, this.DataAttributes.AllowNegativeAxis), drawAxis = accessJQueryData(bar, this.DataAttributes.DrawAxis), containerHeight = containerHeights[i], axisDiv = bar.parentNode && bar.parentNode.querySelector('.' + this.DataAttributes.Axis + '_' + barPrefix);
    ...
    _applyIconSettings(container, iconType, forceLeftAlignment, flexParentStyle = styleSettingsProvider.cssClassNames.flexParent) {
        if (!container.classList.contains('dx-white-space-column')) {
            ...
            wrapInner(container, wrappingElement);
        }
    }
    ...
    _createBarDiv(barInfo, barPrefix) {
        ...
        accessJQueryData(barDiv, this.DataAttributes.NormalizedValue, barInfo.normalizedValue);
        accessJQueryData(barDiv, this.DataAttributes.ZeroPosition, barInfo.zeroPosition);
        accessJQueryData(barDiv, this.DataAttributes.AllowNegativeAxis, barInfo.allowNegativeAxis);
        accessJQueryData(barDiv, this.DataAttributes.DrawAxis, barInfo.drawAxis);
        return barDiv;
    }
    _createAxisDiv(barPrefix, zeroPosition) {
        ...
        accessJQueryData(axisDiv, this.DataAttributes.ZeroPosition, zeroPosition);
        return axisDiv;
    }
    ...
    _createCssClassFromCustomAppearanceType(styleSettingsModel, cssSelector, drawProperty) {
        ...
        this.cssCustomClassesLinks.push(createCssClass(cssSelector, cssProperties, this.container));
    }
    _createFontStyles(styleSettingsModel) {
        ...
        if (styleSettingsModel.ForeColor)
            fontStyles.push(createCssPropertyWrapper('color', toColor(styleSettingsModel.ForeColor)));
        if (styleSettingsModel.FontFamily)
            fontStyles.push(createCssPropertyWrapper('font-family', styleSettingsModel.FontFamily));
        if (styleSettingsModel.FontSize && styleSettingsModel.FontSize > 0)
            fontStyles.push(createCssPropertyWrapper('font-size', styleSettingsModel.FontSize));
        if ((styleSettingsModel.FontStyle & this.FontStyle.Bold) !== 0)
            fontStyles.push(createCssPropertyWrapper('font-weight', 'bold'));
        if ((styleSettingsModel.FontStyle & this.FontStyle.Italic) !== 0)
            fontStyles.push(createCssPropertyWrapper('font-style', 'italic'));
        if (isUnderline && isStrikeout)
            fontStyles.push(createCssPropertyWrapper('text-decoration', 'underline line-through'));
        else if (isUnderline)
            fontStyles.push(createCssPropertyWrapper('text-decoration', 'underline'));
        else if (isStrikeout)
            fontStyles.push(createCssPropertyWrapper('text-decoration', 'line-through'));
        return fontStyles;
    }
    _createBackColorStyle(styleSettingsModel) {
        return styleSettingsModel.Color ? createCssPropertyWrapper('background-color', appearanceSettingsProvider.getColor(styleSettingsModel)) : null;
    }
    _createCssClassFromPredefinedAppearanceType(selector, appearanceType, drawProperty) {
        this.cssCustomClassesLinks.push(createCssClass(selector, appearanceSettingsProvider.toCssProperties(appearanceType, drawProperty), this.container));
    }
}
...
export class cardItemStyleSettingsProvider extends styleSettingsProvider {
    ...
    _getCssTdSelector(cssClassname) {
        ...
        return '.' + styleSettingsProvider.cssClassNames.dashboardContainer + ' .' + layoutCardCssClassNames.card + notSelectedOrHovered + ' .' + cssClassname + ', '
            + '.' + styleSettingsProvider.cssClassNames.dashboardContainer + ' .' + layoutCardCssClassNames.card + notSelectedOrHovered + '.' + cssClassname;
    }
    _applyIconSettings(container, iconType, forceLeftAlignment, flexParentStyle = styleSettingsProvider.cssClassNames.flexParent) {
