...
import { projection as mapProjection } from 'devextreme/viz/vector_map/projection';
import { deepExtend, getHeight, getWidth } from '../../data/_jquery-helpers';
...
var projection = (function () {
    var parameters = mapProjection.get('mercator').source(), _to = parameters.to, _from = parameters.from;
    ...
    return mapProjection(parameters);
...
export class mapItem extends baseItem {
    ...
    _renderContentInternal(element, changeExisting, options) {
        ...
        if (changeExisting && this.mapViewer) {
            ...
            this.mapViewer = new dxVectorMap(element, options);
        }
    ...
    _getSpecificStatePanelItems() {
        if (!this.isViewportLocked && this.isInitialExtentChanged) {
            return [{
                    name: dashboardToolbarItemNames.initialExtent,
                    click: (element) => {
                        this._toggleInitialExtentChanged(false);
                    },
                    icon: cssClasses.iconInitialExtent,
                    type: 'button',
                    hint: localizer.getString(localizationId.buttonNames.InitialExtent)
                }];
        }
        else
            return [];
    ...
    _getMapViewerOptions() {
        ...
        return {
            ...
            zoomFactor: that._calculateZoomFactor(viewModel.Viewport, getWidth(that.contentRoot), getHeight(that.contentRoot)),
            maxZoomFactor: 1 << 18,
    ...
    _getToolTip(name, value) {
        return stringFormat('{0}: {1}', this._getHtml(name), this._getHtml(value));
    }
    ...
    _getViewport() {
        var topLeft = this.mapViewer.convertCoordinates(0, 0), bottomRight = this.mapViewer.convertCoordinates(getWidth(this.contentRoot), getHeight(this.contentRoot)), viewport = this.mapViewer.viewport(), center = this.mapViewer.center();
        return {
    ...
    _getClusterizationInfo() {
        return {
            viewport: this._getViewport(),
            clientSize: {
                width: getWidth(this.contentRoot),
                height: getHeight(this.contentRoot)
            }
        };
    }
    _getWidgetState() {
    ...
    _updateViewport(viewport) {
        this._updateWidgetState({
            zoomFactor: this._calculateZoomFactor(viewport, getWidth(this.contentRoot), getHeight(this.contentRoot)),
