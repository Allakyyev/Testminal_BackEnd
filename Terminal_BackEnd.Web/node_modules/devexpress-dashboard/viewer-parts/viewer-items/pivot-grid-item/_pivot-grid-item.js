...
import { $unwrap, createJQueryCallbacks, createJQueryDeferred, deepExtend } from '../../../data/_jquery-helpers';
import { localizationId } from '../../../data/_localization-ids';
import { localizer } from '../../../data/_localizer';
import { distinct, findIndex, minus, pivotArea } from '../../../data/_utils';
...
import { FieldsExtractor, FilterChecker, OneElementFilterRemover, pivotHelper } from './_pivot-grid-item-helper';
...
export class pivotGridItem extends baseItem {
        ...
        if (this.options) {
            this._createAndLoadDataSource().then(result => {
                ...
                if (changeExisting && this.pivotGridViewer) {
                    ...
                    this.pivotGridViewer = this._createTestableWidget(element, pivotOptions);
                }
                this._conditionalFormattingInfoCache = [];
                afterRenderCallback();
            });
    ...
    _getPivotGridOptions(dataSource, fields) {
        ...
        commonOptions.stateStoring = {
            ...
            savingTimeout: 0,
            customLoad: () => {
                return createJQueryDeferred().resolve(this._loadState(fields)).promise();
            },
        ...
        commonOptions.texts = {
            grandTotal: localizer.getString(localizationId.PivotGridGrandTotal),
            total: localizer.getString(localizationId.PivotGridTotal),
            noData: localizer.getString(localizationId.MessagePivotHasNoData)
        };
    ...
    _resetPivotLoadingDeferred() {
        this._pivotLoadingDeferred && this._pivotLoadingDeferred.reject();
        this._pivotLoadingDeferred = createJQueryDeferred();
    }
    ...
    _getDataPoint(element) {
        ...
        return {
            getValues(name) {
                switch (name) {
                    case itemDataAxisNames.pivotRowAxis:
                        return element.rowPath;
                    case itemDataAxisNames.pivotColumnAxis:
    ...
    _onCellPrepared(element) {
        var styleSettingsInfo, isMeasureHeader = (element.area === pivotArea.column || element.area === pivotArea.row) && element.cell.dataIndex !== undefined, cellItem = {
            area: element.area
        };
        if (!isMeasureHeader) {
            if (element.area === pivotArea.column) {
                cellItem.columnPath = element.cell.path;
                cellItem.columnType = element.cell.type;
            }
            else if (element.area === pivotArea.row) {
            ...
            this._styleSettingsProvider.applyStyleSettings($unwrap(element.cellElement), this.options.controlContainer, styleSettingsInfo, false, PIVOT_BAR_ID);
        }
    }
    _getFields() {
        if (!this._viewModel) {
        ...
        let columnFields = pivotHelper.createAreaFields(this._viewModel.Columns, 'column', this._viewModel.AutoExpandColumnGroups);
        for (var i = 0; i < this._viewModel.Columns.length; i++) {
            columnFields[i].showTotals = this._viewModel.Columns[i].ShowTotals;
        }
        let rowFields = pivotHelper.createAreaFields(this._viewModel.Rows, 'row', this._viewModel.AutoExpandRowGroups);
        for (var i = 0; i < this._viewModel.Rows.length; i++) {
            rowFields[i].showTotals = this._viewModel.Rows[i].ShowTotals;
        }
        let dataFields = pivotHelper.createAreaFields(this._viewModel.Values, 'data');
        ...
        return new Promise((resolve) => {
            ...
            var dataSource = new PivotGridDataSource({
                ...
                load: (loadOptions) => {
                    if (firstRequest) {
                        firstRequest = false;
                        return [pivotHelper.getSchemaRow(fields)];
                    }
                    else {
                        return this._dataUpdateHook ? this._dataUpdateHook.then(() => this._getLoadData(loadOptions)) : createJQueryDeferred().reject().promise();
                    }
                }
            });
    ...
    _getLoadData(loadOptions) {
        ...
        let filter = OneElementFilterRemover.simplify(loadOptions.filter);
        let availableByArea = {
            columns: pivotHelper.getColumnAxis(this._multiData).getDimensions().map(c => c.id),
            rows: pivotHelper.getRowAxis(this._multiData).getDimensions().map(r => r.id)
        };
        let filteredFields = FieldsExtractor.extract(filter);
        let filteredByArea = {
        ...
        let firstIndexInGroup = {
            column: findIndex(groupedFields, f => availableByArea.columns.filter(c => c === f)[0] !== undefined),
            row: findIndex(groupedFields, f => availableByArea.rows.filter(c => c === f)[0] !== undefined)
        };
        let onlyFilteredByArea = {
            columns: distinct(minus(filteredByArea.columns, groupedByArea.columns)),
            rows: distinct(minus(filteredByArea.rows, groupedByArea.rows))
        };
        let fields;
        ...
        let columnRoot = pivotHelper.getColumnAxis(this._multiData).getRootPoint();
        let rowRoot = pivotHelper.getRowAxis(this._multiData).getRootPoint();
        let getItems = (index, isColumn, point, oppositePoint, dimensionValues) => {
            ...
            for (let i = 0; i < children.length; i++) {
                ...
                if (filter && !FilterChecker.fits(dimVals, filter))
                    continue;
                let subItems;
                if (!isLastGrouping) {
                ...
                if (isOnlyFiltered) {
                    ...
                    items.push({
                        key: value,
                        displayText: value === specialValues.olapNullValueGuid ? localizer.getString(localizationId.OlapRaggedHierarchyNoneItemCaption) : child.getDisplayText(),
                        items: subItems,
        ...
        if (groupedFields && groupedFields.length > 0) {
            ...
            res.data = [pivotHelper.getSchemaRow(this._getFields())];
        }
        if (totalSummary)
