...
import { DashboardDataIdField } from '../../../data/_common';
import { $unwrap, $wrap, createJQueryCallbacks, deepExtend, getWidth } from '../../../data/_jquery-helpers';
...
import { DashboardPrivateSettings } from '../../legacy-settings';
import { baseItem } from '../_base-item';
import { ColumnWidthCalculator } from './_column-width-calculator';
import { GridColumnPainter } from './_grid-column-painter';
import { parseFilter } from './_grid-filter-parser';
import { allowedStringOperations, getCaseSensitiveFilterExpression, getFilterRowLocalizationOptions } from './_grid-filter-patcher';
...
export class dataGridItem extends baseItem {
    constructor(container, options) {
        ...
        this.clientFilterChanged = createJQueryCallbacks();
        this.clientFilterStateChanged = createJQueryCallbacks();
        this.TextAlignment = {
            Left: 'left',
            Right: 'right',
            Center: 'center'
        ...
        this._bestFitProvider = {
            getBestFit: (index) => {
                ...
                if (columnViewModel.DisplayMode === this.DisplayMode.Sparkline) {
                    ...
                    let startValues = $unwrap(this._dataGrid.element()).querySelectorAll('.' + columnName + '_startValue');
                    let endValues = $unwrap(this._dataGrid.element()).querySelectorAll('.' + columnName + '_endValue');
                    let maxStartWidth = GridColumnPainter.calcMaxWidth(startValues);
                    let maxEndWidth = GridColumnPainter.calcMaxWidth(endValues);
        ...
        this._clientStateUpdateDebounced = debounce((widthOptions) => {
        ...
        this._styleSettingsProvider = new styleSettingsProvider(this.container);
    ...
    renderContentUnsafe(element, changeExisting, afterRenderCallback) {
        ...
        if (changeExisting && this._dataGrid) {
            ...
            deepExtend(opts, this._getCommonOptions());
            ...
            this._dataGrid = this._createTestableWidget(element, options);
    ...
    getInfoUnsafe() {
        ...
        return deepExtend(super.getInfoUnsafe(), {
            scroll: {
            ...
            combinedFilter: combinedFilter !== undefined ? parseFilter(combinedFilter, visibleColumns, undefined, this.dateToString) : '',
    ...
    _resetColumnWidths() {
        ...
        let columnsWidth = this._calculator.calcColumnsWidth(this._bestFitProvider, ($wrap(this._dataGrid.element())).width() - this._dataGrid.getScrollbarWidth(), this.charWidth);
    ...
    _updateCharWidth() {
        ...
        this.charWidth = getWidth(span) / 10;
    ...
    _getViewOptions() {
        ...
        var viewOptions = {
            ...
            filterRow: Object.assign({ visible: viewModel.ShowFilterRow }, getFilterRowLocalizationOptions()),
    ...
    _getCommonOptions() {
        ...
        commonOptions.sorting = {
            mode: 'multiple',
            ascendingText: localizer.getString(localizationId.buttonNames.GridSortAscending),
            descendingText: localizer.getString(localizationId.buttonNames.GridSortDescending),
            clearText: localizer.getString(localizationId.buttonNames.GridClearSorting)
        };
        if (this.manualyResetClientState) {
            commonOptions.onContextMenuPreparing = (options) => {
                if (options.target === 'content' && this._calculator.columnsResized) {
                    options.items = [{
                            text: localizer.getString(localizationId.buttonNames.GridResetColumnWidths),
        ...
        commonOptions.noDataText = localizer.getString(localizationId.MessageGridHasNoData);
    ...
    _getTotalCaptionTemplate(totalType) {
        if (totalType == 'Auto') {
            return getLocalizationById('DashboardStringId.GridTotalAutoTemplate');
        }
        else {
            let id;
            switch (totalType) {
            ...
            return stringFormat(getLocalizationById('DashboardStringId.GridTotalTemplate'), getLocalizationById(id), getLocalizationById('DashboardStringId.GridTotalValueTemplate'));
        }
    }
    _getTotals() {
    ...
    calculateFilterExpressionHandler(column) {
        return (filterValue, selectedFilterOperation) => {
            ...
            const canPatchFilter = !this.isSummaryFilterCollecting && gridColumn.dataType === 'string' && allowedStringOperations.some(x => x === selectedFilterOperation);
            if (canPatchFilter) {
                return (values) => {
                    const cellValue = values[gridColumn.dataField.toString()];
                    return getCaseSensitiveFilterExpression(cellValue, filterValue, selectedFilterOperation);
                };
            }
    ...
    _getColumns() {
        ...
        columns.forEach((column, columnIndex) => {
            ...
            if (DashboardPrivateSettings.gridColumnFilterMode === 'CaseSensitive')
                gridColumn.calculateFilterExpression = this.calculateFilterExpressionHandler(gridColumn).bind(this);
            gridColumn[DashboardDataIdField] = column.DataId;
            ...
            switch (column.DisplayMode) {
                case that.DisplayMode.Value:
                    gridColumn.cellTemplate = (containerElement, options) => {
                        let container = $unwrap(containerElement);
                        var valueItem = that.getValueItem(fieldName, options.data.index);
                        GridColumnPainter.renderValue(container, valueItem.getData().displayText, that._isEncodeHtml());
                ...
                case that.DisplayMode.Delta:
                    gridColumn.cellTemplate = function (containerElement, options) {
                        ...
                        var valueItem = that.getValueItem(fieldName, options.data.index);
                        let isDetail = that._isDetail(options.rowType);
                        GridColumnPainter.renderDelta(container, valueItem.getData(), isDetail);
                ...
                case that.DisplayMode.Hyperlink:
                    gridColumn.cellTemplate = function (containerElement, options) {
                        let container = $unwrap(containerElement);
                        var valueItem = that.getValueItem(fieldName, options.data.index), uri = fieldName ? stringFormat(!!column.UriPattern ? column.UriPattern : '{0}', valueItem.getUriValue()) : undefined;
                        GridColumnPainter.renderHyperlink(container, uri, valueItem.getData().displayText, that._isEncodeHtml());
                ...
                case that.DisplayMode.Sparkline:
                    ...
                    gridColumn.cellTemplate = (containerElement, options) => {
                        let container = $unwrap(containerElement);
                        var valueItem = that.getValueItem(fieldName, options.data.index);
                        GridColumnPainter.renderSparkline(columnName, column.ShowStartEndValues, container, valueItem.getData());
                        that._styleSettingsProvider.applyStyleSettings(container, popupContainer, valueItem.getStyleSettingsInfo(), true);
                        if (that._isDetail(options.rowType))
                            container.classList.add(that.CssClasses.gridAdaptiveCellValue);
                    };
                ...
                case that.DisplayMode.Bar:
                    ...
                    gridColumn.cellTemplate = function (containerElement, options) {
                        ...
                        GridColumnPainter.renderBar(columnName, container, barData.text, barData.normalizedValue, barData.zeroValue);
                        that._styleSettingsProvider.applyStyleSettings(container, popupContainer, valueItem.getStyleSettingsInfo(), true);
                        if (that._isDetail(options.rowType))
                            container.classList.add(that.CssClasses.gridAdaptiveCellValue);
                    };
                    gridColumn.resized = function () {
                        var gridRootElement = that._dataGrid ? $unwrap(that._dataGrid.element()) : undefined;
                        if (gridRootElement) {
                            GridColumnPainter.changeGridBarColumnsWidth(gridRootElement, columnName);
                        }
                    };
                    break;
                case that.DisplayMode.Image:
                    gridColumn.cellTemplate = function (containerElement, options) {
                        let container = $unwrap(containerElement);
                        var valueItem = that.getValueItem(fieldName, options.data.index);
                        GridColumnPainter.renderImage(container, valueItem.getData());
                        that._styleSettingsProvider.applyStyleSettings(container, popupContainer, valueItem.getStyleSettingsInfo(), true);
                        if (that._isDetail(options.rowType))
                            container.classList.add(that.CssClasses.gridAdaptiveCellValue);
                    };
    ...
    _changeGridSparklineColumnsWidth(columnViewModels) {
        if (columnViewModels) {
            let gridRootElement = this._dataGrid ? $unwrap(this._dataGrid.element()) : undefined;
            if (gridRootElement) {
                ...
                sparklineViewModels.forEach(columnViewModel => {
                    GridColumnPainter.changeGridSparklineColumnsWidth(gridRootElement, this._getColumnName(columnViewModel));
                });
            }
        }
    }
    ...
    _raiseClientFilterChanged(e) {
        if (this.canRaiseFilterChanged) {
            ...
            this.isSummaryFilterCollecting = false;
            let filter = combinedFilter !== undefined ? parseFilter(combinedFilter, visibleColumns, undefined, this.dateToString) : '';
