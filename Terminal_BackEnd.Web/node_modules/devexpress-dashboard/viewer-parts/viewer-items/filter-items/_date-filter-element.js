...
import { convertToFormat } from '../../../data/_formatter';
...
export class dateFilterElement extends baseItem {
    ...
    renderContentUnsafe(element, changeExisting, afterRenderCallback) {
        ...
        if (isFirstInitialization)
            this.widget = new DateFilterWidget(element, { useNativeScrolling: this.options.useNativeScrolling, overflow: this.options.overflow }, this.controlContainer);
        var options = this.getWidgetOptions();
        this._lock();
        this.widget._update(options);
        this._unlock();
        this.widget._updateSize(getWidth(this.contentRoot), getHeight(this.contentRoot));
    ...
    getWidgetOptions() {
        ...
        return {
            ...
            format: (value) => {
                ...
                return DashboardFormatHelper.format(valueToFormat, convertToFormat({ DateTimeFormat: filterViewModel.DateTimeFormat }));
            },
            buttonClick: (name) => {
                if (!this._isLocked()) {
            ...
            valueChanged: values => {
                if (!this._isLocked())
                    this.selected.fire(this.getName(), viewerActions.setMasterFilter, [values]);
            },
    ...
    _updateContentSizeUnsafe() {
        ...
        var newContentWidth = getWidth(this.contentRoot), newContentHeight = getHeight(this.contentRoot);
    ...
    _generateInnerBorderClassesUnsafe(element) {
        ...
        if (!this._isPaneEmpty()) {
            classes.push(cssDateFilterClassNames.item);
        }
        if (element) {
            if (this._isPaneEmpty()) {
                element.classList.remove(cssDateFilterClassNames.item);
            }
            else {
                element.classList.add(cssDateFilterClassNames.item);
            }
        }
        return classes;
    }
