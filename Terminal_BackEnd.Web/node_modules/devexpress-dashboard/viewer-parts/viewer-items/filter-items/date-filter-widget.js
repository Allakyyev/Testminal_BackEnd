...
import { $unwrap, getWidth, wrapPublicElement } from '../../../data/_jquery-helpers';
import { getLocalizationById } from '../../../data/localization/_default';
import { CalendarHelper } from '../../_utils';
import { RenderHelper } from '../../widgets/_render-helper';
import { DatePickerButtons, dateFilterWidgetClassNames } from './_date-filter-widget-options';
...
export class DateFilterWidget {
    constructor(element, viewerOptions, boundaryElementContainer) {
        ...
        this._defaultButtonText = getLocalizationById('DashboardStringId.DateFilterDatePickerButtonDefaultText');
        this._overlayShown = false;
        this._lockSelectionEvents = false;
        this._updateScrollableContainer = () => { };
        this._getBtnsContainerScrollWidth = () => 0;
        this._datePickerContent = new DatePickerButtons();
        this.quickButtons = [];
        this._parent = document.createElement('div');
        element.appendChild(this._parent);
        this._scrollableContent = RenderHelper.wrapScrollable(this._parent, viewerOptions.overflow, 'both');
        this._updateScrollableContainer = () => RenderHelper.updateScrollable(this._parent);
    }
    get calendarFrom() {
    ...
    element() {
        return wrapPublicElement(this._scrollableContent);
    }
    _update(widgetOptions) {
        ...
        this._widgetDiv = this._createDiv(content, dateFilterWidgetClassNames.widget);
        ...
        if (hasQuickFilters) {
            this._options.buttonsInfo.forEach(info => {
                let buttonDiv = this._createDiv(buttonsDiv, dateFilterWidgetClassNames.button);
                var button = new dxButtonGroup(buttonDiv, {
                    items: [{ text: info }],
        ...
        if (this._options.datePickerLocation == 'Far')
            this._addDatePicker(buttonsDiv);
        var overlayDiv = this._createDiv(this._widgetDiv, dateFilterWidgetClassNames.overlayWrapper);
        this._createDropDownContent();
        this._overlay = new dxOverlay(overlayDiv, this._getOverlayOptions());
    ...
    _addDatePicker(parentElement) {
        this._datePickerDiv = this._createDiv(parentElement, dateFilterWidgetClassNames.button);
        this.datePickerButton = new dxButtonGroup(this._datePickerDiv, {
    ...
    _createButtonsDiv() {
        var classNames = [dateFilterWidgetClassNames.buttons];
        if (this._options.arrangementMode === 'Vertical')
            classNames.push(dateFilterWidgetClassNames.buttonsTopDown);
        else
            classNames.push(dateFilterWidgetClassNames.buttonsLeftToRight);
        if (!this._options.hasOuterMargin)
            classNames.push(dateFilterWidgetClassNames.buttonsNoPaddings);
        return this._createDiv(this._widgetDiv, ...classNames);
    }
    _createDropDownContent() {
        var filterType = this._options.filterType;
        this._dropDownContentDiv = this._createDiv(null, dateFilterWidgetClassNames.overlay);
        if (this._options.mobileLayout)
            this._addClass(this._dropDownContentDiv, dateFilterWidgetClassNames.mobile);
    ...
    _createDesktopCalendar(containerDiv, createButton) {
        var div = this._createDiv(containerDiv, dateFilterWidgetClassNames.overlayCalendarContainer);
        ...
        return new PopupCalendarWidgets(calendar, dateBox, submitButton);
    }
    _createMobileCalendar(containerDiv, setDate, createButton) {
        ...
        var dateBox = this._createTextBox(div, textBoxValueChanged);
        var submitButton = createButton ? this._createSubmitButton(div) : null;
        return new PopupCalendarWidgets(dateBox, submitButton);
    }
    _createCalendar(containerDiv, valueChanged) {
        var div = this._createDiv(containerDiv, dateFilterWidgetClassNames.overlayCalendar);
        return new dxCalendar(div, {
            focusStateEnabled: false,
            maxZoomLevel: CalendarHelper.getCalendarMaxZoomLevel(this._options.groupInterval),
            onValueChanged: valueChanged
        });
    }
    _createTextBox(containerDiv, textBoxValueChanged) {
        var div = this._createDiv(containerDiv, dateFilterWidgetClassNames.overlayTextBox);
        return new dxDateBox(div, {
            width: 'auto',
    ...
    _createSubmitButton(containerDiv) {
        var div = this._createDiv(containerDiv, dateFilterWidgetClassNames.overlayButton);
        return new dxButton(div, {
            text: getLocalizationById('DashboardStringId.ButtonApply'),
            onClick: (e) => {
                this._submit();
            }
        });
    ...
    _updateDropDownButtonText() {
        ...
        if (this.datePickerButton) {
            if (start || end) {
                ...
                if (!formatPatten) {
                    this._setDatePickerText(constructIntervalFilterText({ left: startText, right: endText }), true);
                }
                else
                    this._setDatePickerText(stringFormat(formatPatten, startText, endText), true);
            }
            else {
    ...
    _getDefaultOptions() {
        return {
            ...
            buttonsInfo: [],
            displayTextPattern: getLocalizationById('DashboardStringId.FromToDatePeriodCaption'),
    ...
    _getOverlayOptions() {
        ...
        const options = {
            ...
            contentTemplate: (contentElement) => {
                $unwrap(contentElement).appendChild(this._dropDownContentDiv);
            },
            hideOnOutsideClick: true,
            wrapperAttr: {
                class: dateFilterWidgetClassNames.overlayWrapper
            }
        };
        options.container = that.boundaryElementContainer;
        options.animation = false;
        return options;
    }
    _getOverlayWidth() {
        if (this._options.mobileLayout)
            return function () { return getWidth(window) * 0.9; };
        return 'auto';
    }
    _setDatePickerText(text, showDropDown) {
        if (!showDropDown)
            this._addClass(this._datePickerDiv, dateFilterWidgetClassNames.datePickerButtonEmpty);
        else
            this._removeClass(this._datePickerDiv, dateFilterWidgetClassNames.datePickerButtonEmpty);
