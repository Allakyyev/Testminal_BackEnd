...
import { contentType, viewerActions } from '../../data/_common';
import { defaultDataControllerFactory } from '../../data/_factory';
...
import { checkArrayContainsTuple, checkValuesAreEqual, encodeHtml, getAxisPointValue, moveContent, type } from '../../data/_utils';
import { zIndex } from '../../data/_z-index';
import { constraints, size } from '../layout/_utils.layout';
import { DashboardViewerConstants } from '../viewer/_dashboard-viewer-constants';
...
import { ClickableFloatingCaptionToolbar } from '../widgets/caption-toolbar/_clickable-floating-toolbar';
import { HiddenCaptionToolbar } from '../widgets/caption-toolbar/_hidden-caption-toolbar';
import { HoveredFloatingCaptionToolbar } from '../widgets/caption-toolbar/_hovered-floating-toolbar';
import { HoveredDashboardCaptionToolbar } from '../widgets/caption-toolbar/_hovered-toolbar';
import { MinimizedClickableCaptionToolbar } from '../widgets/caption-toolbar/_minimized-clickable-toolbar';
import { MinimizedHoveredCaptionToolbar } from '../widgets/caption-toolbar/_minimized-hovered-toolbar';
import { StaticCaptionToolbar } from '../widgets/caption-toolbar/_static-toolbar';
import { RenderHelper } from '../widgets/_render-helper';
import { DashboardLayoutModeHelper } from '../_dashboard-layout-mode-helper';
import { ItemLoadingElement } from './elements/_item-loading';
import { dashboardSelectionMode, interactivityController } from './_interactivity-controller';
export var createDefaultToolbar = (viewerItem, container, controlContainer, popupContainer, viewOptions) => {
    if (viewOptions.hiddenToolbar) {
        return new HiddenCaptionToolbar();
    }
    else if (DashboardLayoutModeHelper.isTouch) {
        if (viewOptions.hasCaption) {
            return new StaticCaptionToolbar(container, controlContainer, popupContainer, viewOptions.encodeHtml, cssClasses.caption, viewOptions.captionToolbarSeparatorRequired, false);
        }
        else {
            if (viewOptions.allowPreview) {
                return new MinimizedClickableCaptionToolbar(container, controlContainer, popupContainer, viewOptions.encodeHtml, viewOptions.isBottomFloatingToolbarPosition, viewOptions.itemHasOwnContent);
            }
            else {
                return new ClickableFloatingCaptionToolbar(container, controlContainer, popupContainer, viewOptions.encodeHtml, viewOptions.isBottomFloatingToolbarPosition, viewOptions.itemHasOwnContent);
            }
        }
    }
    else {
        if (viewOptions.hasCaption) {
            return new HoveredDashboardCaptionToolbar(container, controlContainer, popupContainer, viewOptions.encodeHtml, viewOptions.captionToolbarSeparatorRequired);
        }
        else {
            if (viewOptions.allowPreview) {
                return new MinimizedHoveredCaptionToolbar(container, controlContainer, popupContainer, viewOptions.encodeHtml, viewOptions.isBottomFloatingToolbarPosition);
            }
            else {
                return new HoveredFloatingCaptionToolbar(container, controlContainer, popupContainer, viewOptions.encodeHtml, viewOptions.isBottomFloatingToolbarPosition);
            }
        }
    }
};
export function getControlContainer(element) {
...
export class baseItem {
    constructor($container, options) {
        ...
        this.selected = createJQueryCallbacks();
        this.clearMasterFilter = createJQueryCallbacks();
        this.drillUp = createJQueryCallbacks();
        this.contentElementSelection = createJQueryCallbacks();
        this.expandValue = createJQueryCallbacks();
        this.clientStateUpdate = createJQueryCallbacks();
        this.dataRequest = createJQueryCallbacks();
        this.itemClick = createJQueryCallbacks();
        this.itemHover = createJQueryCallbacks();
        this.itemSelectionChanged = createJQueryCallbacks();
        this.itemWidgetCreated = createJQueryCallbacks();
        this.itemWidgetUpdating = createJQueryCallbacks();
        this.itemWidgetUpdated = createJQueryCallbacks();
        this.itemWidgetOptionsPrepared = createJQueryCallbacks();
        this.itemCaptionToolbarUpdated = createJQueryCallbacks();
        this.constraintsUpdated = createJQueryCallbacks();
        this.customTargetAxes = [];
        this.customDefaultSelectedValues = [];
        this.itemLoadingElement = new ItemLoadingElement();
        this.visualMode = 'full';
        this._hasWidget = false;
        this.deferredToolbarRenderingPromise = undefined;
        this.createCaptionToolbar = createDefaultToolbar;
        ...
        this.interactivityController = new interactivityController($.proxy(this.getSelectedTuples, this));
        this.interactivityController.selectionChanged.add($.proxy(this._onSelectionChanged, this));
        this.customSelectionMode = dashboardSelectionMode.none;
    }
    get hasWidget() {
    ...
    _initializeData(newOptions) {
        ...
        if (!this.dataController || (newOptions.ContentType != contentType.actionModel)) {
            var drillDownState = {};
            drillDownState[this._getDrillDownAxisName()] = this.options.DrillDownUniqueValues;
            this.dataController = defaultDataControllerFactory.createDataController(this.options.Type, {
    ...
    updateItem(options) {
        ...
        if (!this.isInteractivityActionEnabled()) {
            var customDefaultSelectedValues = options.selectionMode == dashboardSelectionMode.single ? that.customDefaultSelectedValues.slice(0, 1) : that.customDefaultSelectedValues;
    ...
    _onSelectionChanged(tuples) {
        ...
        if (this.isInteractivityActionEnabled()) {
            tuples.forEach(tuple => {
                ...
                that._getAxisNames().forEach(axisName => {
                    tupleValues.push.apply(tupleValues, getAxisPointValue(tuple, axisName).slice());
                });
                newSelection.push(tupleValues);
            });
            newSelection = this._deductDrillDownValues(newSelection);
            this._setSelectedValues(newSelection);
            actionName = this._getSelectionCallbackType(newSelection.length === 1 && singleSelection && checkValuesAreEqual(newSelection[0], singleSelection));
            if (actionName === viewerActions.drillDown) {
        ...
        if (this.customSelectionMode == dashboardSelectionMode.multiple) {
            this.updateCaptionToolbar();
        }
    ...
    _deductDrillDownValues(values) {
        ...
        values.forEach(value => {
            ...
            if (checkValuesAreEqual(cutValue, drillDownValues))
    ...
    _selectTuples(tuplesToSelect, unaffectedTuples, isSelect) {
        ...
        var updateTuple = function (tuple) {
            if (checkArrayContainsTuple(unaffectedTuples, tuple) == undefined) {
    ...
    hasParentContainer() {
        return this.options && type.isDefined(this.options.ParentContainer);
    }
    _isPaneEmpty() {
    ...
    render(container) {
        ...
        moveContent(this.container, this.contentRoot, true);
    ...
    updateContent(newOptions) {
        ...
        if (type.isDefined(isNewShowCaption) && showCaptionChanged) {
            this.forceCreateCaptionToolbar();
        }
        else {
            this.updateCaptionToolbar();
        ...
        switch (newOptions.ContentType) {
            case contentType.partialDataSource:
            case contentType.completeDataSource:
    ...
    updateClientState(clientState) {
        if (this.hasWidget && this.options.ContentType !== contentType.partialDataSource && this.options.ContentType !== contentType.completeDataSource)
    ...
    _toggleLoadingPanel(loading) {
        ...
        if (!loading) {
            ...
            shieldingElement.style.zIndex = zIndex.dashboardItemShield.toString();
    ...
    getConstraints(includeBorders) {
        var borderSize = includeBorders ? RenderHelper.getBorderSizeByClasses(this._generateOuterBorderClasses().concat(this._generateInnerBorderClasses())) : { width: 0, height: 0 }, headerHeight = this._calcHeaderAndFooterHeight(), contentMinHeight = this._getMinContentHeight(), height = borderSize.height + headerHeight + contentMinHeight;
        return constraints(size(DashboardViewerConstants.minPaneWidth + borderSize.width, height), size(Number.MAX_VALUE, this._isFixedHeight ? height : Number.MAX_VALUE));
    }
    getOffset() {
        return {
            width: 0,
    ...
    updateInteractivityOptions() {
        var that = this, selectionMode = dashboardSelectionMode.none;
        if (!that.isInteractivityActionEnabled()) {
            that.updateContentState();
            selectionMode = this.customSelectionMode;
            if (selectionMode == dashboardSelectionMode.multiple && !this.allowMultiselection)
                selectionMode = dashboardSelectionMode.single;
        }
        else {
            if (this._canSetMultipleMasterFilter() && this.allowMultiselection) {
                selectionMode = dashboardSelectionMode.multiple;
            }
            else {
                if (that.isInteractivityActionEnabled()) {
                    selectionMode = dashboardSelectionMode.single;
    ...
    _getReducedDataTooltip() {
        if (this.options.LimitDataState) {
            if (this.options.LimitDataState.isReduced)
                return getLocalizationById('DashboardWebStringId.LimitVisibleData.DisplayAllDataTooltip');
            return getLocalizationById('DashboardWebStringId.LimitVisibleData.IncreasePerformanceTooltip');
        }
        return undefined;
    }
    _getStaticToolbarItems() {
        ...
        if (this.hasCaption()) {
            ...
            if (caption) {
                items.push({
                    name: dashboardToolbarItemNames.itemCaption,
                    type: 'text',
                    text: caption
                });
            }
    ...
    _getReduceDataToolbarItem() {
        return {
            name: dashboardToolbarItemNames.limitVisibleData,
            checked: this.options.LimitDataState.isReduced,
            click: (element) => { this.allowLimitDataCallback(); },
            icon: cssClasses.iconLimitVisibleData,
            type: 'button',
            tooltip: {
                className: cssClasses.tooltipLimitVisibleData,
                template: () => {
    ...
    _getStateToolbarItems() {
        ...
        if (this._hasClearMasterFilterButton()) {
            items.push({
                name: dashboardToolbarItemNames.clearMasterFilter,
                click: (element) => { this._onClearMasterFilter(); },
                icon: cssClasses.iconClearMasterFilter,
                type: 'button',
                hint: localizer.getString(localizationId.buttonNames.ClearMasterFilter)
            });
        }
        ...
        if (this._hasClearSelectionButton())
            items.push({
                name: dashboardToolbarItemNames.clearSelection,
                click: (element) => { this._onClearSelection(); },
                icon: cssClasses.iconClearSelection,
                type: 'button',
                hint: localizer.getString(localizationId.buttonNames.ClearSelection)
            });
        if (this._hasDrillUpButton() && this._isDrillUpEnabled())
            items.push({
                name: dashboardToolbarItemNames.drillUp,
                click: (element) => { this._onDrillUp(); },
                icon: cssClasses.iconDrillUp,
                type: 'button',
                hint: localizer.getString(localizationId.buttonNames.DrillUp)
            });
    ...
    _getActionToolbarItems() {
        ...
        if (contentDescription && contentDescription.ElementSelectionEnabled)
            items.push({
                name: dashboardToolbarItemNames.values,
                menu: {
                ...
                icon: cssClasses.iconContentSelection,
                type: 'menu',
                hint: localizer.getString(localizationId.buttonNames.ElementSelection)
            });
        if (this._hasToggleSelectionModeButton())
            items.push({
                name: dashboardToolbarItemNames.multiselection,
                checked: this.allowMultiselection,
                click: (element) => { this._onToggleSelectionMode(); },
                icon: cssClasses.iconMultiselection,
                type: 'button',
                hint: localizer.getString(localizationId.buttonNames.AllowMultiselection)
            });
        return items;
    }
    _getExportMenuModel() {
        let items = [cssClasses.iconExportToPDF];
        if (Settings.allowExportToImage)
            items.push(cssClasses.iconExportToImage);
        if (this._isSupportDataAwareExport()) {
            items.push(cssClasses.iconExportToExcel);
        }
        return {
            title: localizer.getString(localizationId.buttonNames.ExportTo),
            items: items,
            itemClick: (itemData, itemElement, index) => { this.showExportDialog(_convertToExportFormat(itemData)); },
            type: 'icons'
        };
    }
    _getParametersTitle() {
        ...
        if (drillDownValues && drillDownValues.length > 0) {
            parameters = parameters.concat(drillDownValues.map(drillDownValue => format(drillDownValue.Value, drillDownValue.Format)));
        }
    ...
    _getMinContentHeight() {
        return DashboardViewerConstants.minPaneHeight;
    }
    ...
    _generateInnerBorderClassesUnsafe(element) {
        var classes = [cssClassNamesBaseItem.item];
        if (this._isBorderRequired()) {
            classes.push(cssClassNamesBaseItem.simpleBorder);
        }
        if (element) {
            element.classList.add(cssClassNamesBaseItem.item);
            if (this._isBorderRequired()) {
                element.classList.add(cssClassNamesBaseItem.simpleBorder);
            }
            else {
                element.classList.remove(cssClassNamesBaseItem.simpleBorder);
            }
        }
        return classes;
    }
    _generateOuterBorderClasses(element) {
        ...
        if (element) {
            if (this.hasParentContainer() && this.visualMode === 'full') {
                element.classList.add(cssClassNamesBaseItem.groupItemChild);
            }
            else {
                element.classList.remove(cssClassNamesBaseItem.groupItemChild);
            }
        }
        return this.hasParentContainer() && this.visualMode === 'full' ? [cssClassNamesBaseItem.groupItemChild] : [];
    }
    _isBorderRequired() {
    ...
    _getButtonOffset(useToolbarOffset) {
        ...
        var hoverDiv = useToolbarOffset ? this.container.querySelector('.' + cssClassNamesBaseItem.overlayContent) : undefined;
        return {
    ...
    _hasClearSelectionButton() {
        return !this.isInteractivityActionEnabled() && this.customSelectionMode == dashboardSelectionMode.multiple && this.customSelectedTuples.length > 0;
    }
    _hasToggleSelectionModeButton() {
        return this._canSetMultipleMasterFilter() || (!this.isInteractivityActionEnabled() && this.customSelectionMode == dashboardSelectionMode.multiple);
    }
    _isDrillUpEnabled() {
        var actionModel = this.options.ActionModel;
    ...
    _canPerformDrillDown() {
        return this._canPerformAction(viewerActions.drillDown);
    }
    _canPerformDrillUp() {
        return this._canPerformAction(viewerActions.drillUp);
    }
    _canSetMasterFilter() {
        return this._canPerformAction(viewerActions.setMasterFilter);
    }
    _canSetMultipleMasterFilter() {
        return this._canPerformAction(viewerActions.setMultipleValuesMasterFilter);
    }
    ...
    _getHtml(text) {
        return this._isEncodeHtml() ? encodeHtml(text) : text;
    }
    _getAxisNames() {
