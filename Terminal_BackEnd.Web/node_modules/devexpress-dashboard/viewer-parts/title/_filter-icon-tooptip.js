...
export class FilterIconTooltip {
    static getTooltipContent(contentElement, masterFilterValues) {
        contentElement.addClass(titleTooltipClasses.root);
        let $tooltipList = $.fn.constructor('<ul/>', { 'class': titleTooltipClasses.list });
        masterFilterValues.forEach((argument, i) => {
            ...
            RenderHelper.html($.fn.constructor('<div/>', { 'class': titleTooltipClasses.listText }).appendTo($tooltipListItem).get(0), argument.Name, true);
            let $tooltipSublist = $.fn.constructor('<ul/>', { 'class': titleTooltipClasses.subList }).appendTo($tooltipListItem);
            ...
            for (var j = 0; j < maxFilterValues; j++) {
                if (j >= argument.Values.length && argument.Truncated
                    || j + 1 == maxFilterValues && (j + 1 < argument.Values.length
                        || argument.Truncated)) {
                    $.fn.constructor('<li/>', { 'class': titleTooltipClasses.subListItem }).appendTo($tooltipSublist).append('...');
                    break;
                }
                if (j >= argument.Values.length) {
                    break;
                }
                RenderHelper.html($.fn.constructor('<li/>', { 'class': titleTooltipClasses.subListItem }).appendTo($tooltipSublist).get(0), formatFilterValue(argument.Values[j]), true);
            }
        });
        return $tooltipList;
    }
    static _calcMaxFilterListValues(masterFilterValues) {
        var that = this, lineHeightString = $.fn.constructor('<div/>', { 'class': titleTooltipClasses.root }).css('line-height'), lineHeightTmp = parseInt(lineHeightString, 10), lineHeight = lineHeightTmp && lineHeightString.length > 2 && lineHeightString.substr(lineHeightString.length - 2) == 'px' ? lineHeightTmp : this.DEFAULT_LINE_HEIGHT, maxHeight = this._calcMaxHeight();
