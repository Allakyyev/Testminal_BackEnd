...
import { ArrangementInfo, positioningDirection } from './_arrangement-info';
import { cssClassNames } from './_css-class-names';
import { WidgetItem } from './_widget-item';
import { WidgetsViewerBase } from './_widgets-viewer-base';
import { CardItem } from './cards/_card-item';
var viewerCount = 0;
export class dxWidgetsViewer extends WidgetsViewerBase {
    ...
    _init() {
        ...
        this.initContainer();
        this._content = RenderHelper.wrapScrollable(this.container, viewer.overflow, 'both');
    ...
    initContainer() {
        ...
        $unwrap(this.element()).appendChild(this.container);
        this.container.classList.add(cssClassNames.widgetsViewerScrollableHolder);
    }
    clear() {
    ...
    _getScrollable() {
        return RenderHelper.getScrollable(this.container);
    }
    _updateScrollable() {
        RenderHelper.updateScrollable(this.container);
    }
    _scrollTo(left, top) {
    ...
    getSizeParams() {
        ...
        let scrollableContent = this.container.querySelector('.' + cssClassNames.widgetViewerTable);
    ...
    _createItems() {
        ...
        data.forEach(dataItem => {
            ...
            this.itemsList.push((this._widgetType == 'card') ? new CardItem(dataItem, itemOptions) : new WidgetItem(dataItem, itemOptions));
        });
    ...
    _render(drawOptions) {
        ...
        let scrollOffset = this._scrollOffset();
        let animationEnabled = supportAnimation && !contentElement.innerHTML && this.itemsList.length <= DashboardViewerConstants.elementsCountAnimationThreshold;
        ...
        if (table) {
            ...
            this.innerContainer.classList.add(cssClassNames.widgetViewerContainer);
    ...
    _verticalCentering(table) {
        var that = this, differenceTop = that._parentHeight() - $.fn.constructor(that.container.querySelector('.' + cssClassNames.widgetViewerContainer)).height();
        if (differenceTop > 0) {
    ...
    _calcItemIndex(i, j, direction, rowCount, columnCount) {
        return direction == positioningDirection.Horizontal ?
    ...
    _createTable(arrangementInfo, enableAnimation) {
        ...
        for (i = 0; i < rowCount; i++) {
            ...
            row.classList.add(cssClassNames.widgetViewerRow);
            ...
            for (j = 0; j < columnCount; j++) {
                ...
                col.classList.add(cssClassNames.widgetViewerCell);
    ...
    _createArrangementInfo(width, lineCount, itemMinWidth, proportions, direction) {
        ...
        return new ArrangementInfo(that.itemsList.length, lineCount, itemWidth, itemHeight, this._option.itemOptions.itemMargin, direction, options);
    }
    ...
    getItemByIndex(index) {
        ...
        if (type.isNumeric(index)) {
            indexForCheck = Number(index);
            result = this.itemsList[indexForCheck];
        }
        return type.isDefined(result) ? result : null;
    }
    _getScrollBarWidth() {
        ...
        var scrollableContent = RenderHelper.wrapScrollable(container, this._option.viewer.overflow, 'both');
    ...
    _processBatchItems(functionName, startIndex, enableAnimation) {
        ...
        that._styleTag = addToStyles(itemsStyle, this.container);
